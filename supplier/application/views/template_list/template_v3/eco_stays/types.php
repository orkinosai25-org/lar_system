   ...
   <th>Action</th>
    ...
    if (valid_array($table_data) == true) {
        ...
        foreach ($table_data as $k => $v) {
			...
			<td>' . $v['name'] . '</td>';
if ($v['image']!='') {
          $table .= '<td><img width="20%" src="'.base_url().$GLOBALS['CI']->template->domain_eco_stays_images_upload_dir('types/' .$v['image']).'"></td>';
} else {
	   $table .= '<td></td>';
}
...
{
    return '<a role="button" href="' . base_url() . 'index.php/eco_stays/types/' . $id . '" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
