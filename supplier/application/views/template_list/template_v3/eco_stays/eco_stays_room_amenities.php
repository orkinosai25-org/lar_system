            ...
            <div class="panel-title">
                <ul class="nav nav-tabs nav-justified master_tabs" role="tablist" id="myTab">
                    ...
                    <li role="presentation" class="<?php echo $tab2; ?>"><a href="#tableList" aria-controls="profile"
                            role="tab" data-toggle="tab"> Eco Stays Room Amenities List </a></li>
   ...
   <th>Action</th>
    ...
    if (valid_array($table_data) == true) {
        ...
        foreach ($table_data as $k => $v) {
			...
			<td>' . $v['name'] . '</td>';
if ($v['image']!='') {
          $table .= '<td><img width="20%" src="'.base_url().$GLOBALS['CI']->template->domain_eco_stays_images_upload_dir('eco_stays_room_amenities/' .$v['image']).'"></td>';
} else {
	   $table .= '<td></td>';
}
...
{
    return '<a role="button" href="' . base_url() . 'index.php/eco_stays/eco_stays_room_amenities/' . $id . '" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
