...
Class Hotel_Model extends CI_Model
{
	 ...
	 *
	 */
	 public function get_hotel_city_list_old(string $search_chars): array
	{
		$raw_search_chars = $this->db->escape($search_chars);
		$r_search_chars = $this->db->escape($search_chars.'%');
		$search_chars = $this->db->escape('%'.$search_chars.'%');
    ...
    public function get_monthly_booking_summary(array $condition = []): array
	{
		...
		$query = 'select count(distinct(BD.app_reference)) AS total_booking,
				...
				BD.domain_origin='.get_domain_auth_id().' AND BD.created_by_id='.$GLOBALS['CI']->entity_user_id.' AND BD.status = "BOOKING_CONFIRMED" '.$condition.'
	 ...
	 */
	 public function hotel_pnr_data(string $pnr): array{
		$cols = '
...
/**
	 * return booking list
	 */
	function booking(array $condition = [], bool $count = false, int $offset = 0, int $limit = 100000000000): array
	{
		$response=[];
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					where BD.domain_origin='.get_domain_auth_id().' and BD.supplier_id ='.$GLOBALS['CI']->entity_user_id.' '.$condition;
	 ...
	 * return booking list
	 */
	function filter_booking_report(string $search_filter_condition = '', bool $count = false, int $offset = 0, int $limit = 100000000000): array
	{$response=[];
