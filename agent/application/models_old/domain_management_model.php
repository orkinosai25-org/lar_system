...
class Domain_Management_Model extends Abstract_Management_Model
{
	  ...
	  public function master_transaction_request_list(array $data_list_filt = [], string $type = ''): array
    {
        ...
        $query = !empty($type)
            ? "SELECT * FROM master_transaction_details MTD WHERE MTD.type='b2b' AND MTD.created_by_id={$user_id} AND transaction_type='{$type}' {$data_list_cond} ORDER BY origin DESC"
        ...
        if ($this->verify_domain_balance) {
            if ($amount > 0) {
                $this->save_transaction_details(
                    $transaction_type, $app_reference, $amount, $domain_markup,
                    $level_one_markup, $remarks, $convinence, $discount, $currency, $currency_conversion_rate
                );
    ...
    private function get_agent_price(string $model, string $formatter_method, string $app_reference): float
    {
        ...
        $formatted = $this->booking_data_formatter->$formatter_method($booking_data, 'b2b');
    ...
    public function update_agent_balance(float $amount): float
    {
        ...
        if ($details['status'] && !empty($details['data'][0])) {
            ...
            $this->custom_db->update_record('b2b_user_details', $data, $cond);
    ...
    {
        $query = 'SELECT balance AS closing_balance FROM b2b_user_details WHERE user_oid = ' . $user_oid;
    ...
    public function agent_account_ledger(array $condition = [], bool $count = false, int $offset = 0, int $limit = PHP_INT_MAX): array
    {
        ...
        $agent_filter = " AND U.user_id = {$user_id} AND U.user_type = " . B2B_USER;
