...
class Management extends CI_Controller
{
	...
	public function set_balance_alert(): void
	{
		...
		if (valid_array($post_data)) {
			...
			if (isset($post_data['threshold_amount'])) {
				...
				if ($this->form_validation->run()) {
					...
					if ($origin > 0) {
						// UPDATE
						$this->custom_db->update_record('agent_balance_alert_details', $agent_balance_alert_details, [
			...
			if ($page_type == "logo_upload") {
				...
				if ((int)$post_data['origin'] == get_domain_auth_id() && get_domain_auth_id() > 0) {
					...
					if (valid_array($_FILES) && $_FILES['domain_logo']['error'] == 0 && $_FILES['domain_logo']['size'] > 0) {
						...
						// Delete old logo
						$temp_record = $this->custom_db->single_table_records('b2b_user_details', 'logo', ['user_oid' => (int)$this->entity_user_id]);
						...
						if (!$this->upload->do_upload('domain_logo')) {
							...
							$image_data = $this->upload->data();
							$this->custom_db->update_record('b2b_user_details', ['logo' => $image_data['file_name']], ['user_oid' => (int)$this->entity_user_id]);
						}
		...
		// Load domain logo
		$temp_details = $this->custom_db->single_table_records('b2b_user_details', 'logo', ['user_oid' => (int)$this->entity_user_id]);
