...
class application
{
	...
	function initialize_domain_key()
	{
		...
		if (valid_array ( $domain_details ) == true) {
			...
			$this->CI->application_default_template = $_GET['theme'] ?? $domain_details['b2b_theme_id'] ?? $domain_details['theme_id'];
	...
	{
		$skip_validation_list = array('forgot_password', 'agentRegister', 'get_state_list','get_city_lists','back_button','check_otp','send_otp'); //SKIP LIST
		...
		if ($this->skip_validation == false) {
			...
			empty($email) == false and empty($password) == false) {
				...
				if ($this->CI->form_validation->run()) {
					...
					/** USER TYPES **/
					$user_types = array(ULTRALUX_USER);
