...
class Flight extends CI_Controller {
	 ...
	 */
	function pre_calendar_fare_search(): void
{
    $page_params = [];
    ...
    if ($safe_search_data['is_domestic_one_way_flight'] == true) {
	...
	function add_days_todate(): void
{
	 ...
	 */
	function pre_fare_search_result(): void
{
    $get_data = $this->input->get();

    if (!isset($get_data['from']) || empty($get_data['from']) ||
        !isset($get_data['to']) || empty($get_data['to']) ||
        !isset($get_data['depature']) || empty($get_data['depature'])) {
        $this->template->view('general/popup_redirect');
        return;
    }
	...
	function search(int $search_id): void
{
    ...
    if (!valid_array($active_booking_source) || $safe_search_data['status'] !== true) {
    ...
    // Need to check if it's domestic travel
    ...
    $page_params['is_domestic_one_way_flight'] = true;
...
private function fare_quote_booking(array $flight_booking_details): array
{
	 ...
	 */
	private function extra_service_price(array $post_params): float
	{
	...
	function cancel_booking(): void
{
    ...
    if (!isset($post_data['app_reference'], $post_data['booking_source'], $post_data['transaction_origin'], $post_data['passenger_origin']) ||
        !valid_array($post_data['transaction_origin']) || !valid_array($post_data['passenger_origin'])) {
        redirect('security/log_event?event=Invalid Details');
        return;
    }
...
function cancellation_details(string $app_reference, string $booking_source, string $cancellation_status)
{
    if (empty($app_reference) || empty($booking_source)) {
        redirect('security/log_event?event=Invalid Details');
        return;
	...
	public function ticket_cancellation_details(): void
{
    ...
    if (!isset($get_data['app_reference'], $get_data['booking_source'], $get_data['status'])) {
        redirect(base_url());
        return;
    }
	...
	public function cancellation_refund_details(): void
{
    ...
    if (!isset($get_data['app_reference'], $get_data['booking_source'], $get_data['passenger_status'], $get_data['passenger_origin']) ||
        $get_data['passenger_status'] !== 'BOOKING_CANCELLED' || intval($get_data['passenger_origin']) <= 0) {
        redirect(base_url());
        return;
    }
    ...
    if ($notification == 'SEAT NOT AVAILABLE' || $notification == 'seat no available') {
        $message = 'Please book another flight and try again';
    }
    if ($notification == 'Sell Failure') {
        $message = 'Please try again for the same criteria';
    }
    if ($notification == 'The requested class of service is sold out.') {
        $message = 'Please try another booking';
    }
    if ($notification == 'Supplier Interaction Failed while adding Pax Details. Reason: 18|Presentation|Fusion DSC found an exception !\n\tThe data does not match the maximum length: \n\tFor data element: freetext\n\tData length should be at least 1 and at most 70\n\tCurrent position in buffer') {
        $message = 'Please add more than 2 characters in the name field and try agian';
    }
    if ($notification == 'Agency do not have enough balance.') {
	...
	function event_logger($exception = ''): void {
