    ...
    */
	...
	public function dashboard(): void
	{
		...
		if (web_page_access_privilege('p1')) {
            ...
            ///  new month wise report concept

			


 
            $filter = $get_data['months'];
			...
			switch ($filter) {
				case '1':
					// This month: from first day of current month to now
					$startDate = $today->modify('first day of this month')->format('Y-m-d 00:00:00');
					$endDate = (new DateTime())->format('Y-m-d 23:59:59'); // now (end of today)
					$condition[] = array('BD.created_datetime >=', $this->db->escape($startDate));
					$condition[] = array('BD.created_datetime <=', $this->db->escape($endDate));
					break;

				case '3':
					// Last 3 months: from first day of month 3 months ago to end of current month
					$startDate = (new DateTime())->modify('first day of -2 months')->format('Y-m-01 00:00:00'); // 3 months including current is last 3 calendar months
					$endDate = (new DateTime())->modify('last day of this month')->format('Y-m-d 23:59:59');
					$condition[] = array('BD.created_datetime >=', $this->db->escape($startDate));
					$condition[] = array('BD.created_datetime <=', $this->db->escape($endDate));
					break;

				case '6':
					// Last 6 months: from first day of month 6 months ago to end of current month
					$startDate = (new DateTime())->modify('first day of -5 months')->format('Y-m-01 00:00:00');
					$endDate = (new DateTime())->modify('last day of this month')->format('Y-m-d 23:59:59');
					$condition[] = array('BD.created_datetime >=', $this->db->escape($startDate));
					$condition[] = array('BD.created_datetime <=', $this->db->escape($endDate));
					break;
				...
				default:
					// Default: No date filter or you can add your other condition here
					break;
			}
		...
		$page_data['latest_transaction'] = $latest_transaction['data']['transaction_details'];
			...
			$page_data['things_to_do'] = $this->hotel_model->things_to_do_list();
