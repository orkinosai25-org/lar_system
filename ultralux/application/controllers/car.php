...
$currency_obj_conv = new Currency([
			'module_type' => 'car',
			'from' => $currency,
			'to' => admin_base_currency(),
		]);
		
		$agent_payable_amount = $currency_obj_conv->get_agent_paybleamount_car($verification_amount,$currency);
		
		$domain_balance_status = $this->domain_management_model->verify_current_balance($agent_payable_amount['amount'], admin_base_currency());
	...
	public function secure_booking(): void
	{
		...
		$agent_payable_amount = $currency_obj->get_agent_paybleamount_car($amount,$currency);
