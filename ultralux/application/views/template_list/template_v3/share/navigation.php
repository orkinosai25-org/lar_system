	...
	<?php
		...
		foreach ( $master_module_list as $k => $v ) {
			if (in_array ( $k, $active_domain_modules )) {
				if($v == 'flights') {//FIXME: remove later
			?>
			<li
				class="<?=((@$default_view == $k || $default_view == $v) ? 'active' : '')?>"><a
				href="<?php echo base_url()?>index.php/report/flight"> <?php echo ucfirst($v)?></a>
			</li>
	<?php }elseif($v == 'hotels') {//FIXME: remove later
			?>
			<li
				class="<?=((@$default_view == $k || $default_view == $v) ? 'active' : '')?>"><a
				href="<?php echo base_url()?>index.php/report/hotel"> <?php echo ucfirst($v)?></a>
			</li>
	<?php }elseif($v == 'cars') {//FIXME: remove later
			?>
			<li
				class="<?=((@$default_view == $k || $default_view == $v) ? 'active' : '')?>"><a
				href="<?php echo base_url()?>index.php/report/car"> <?php echo ucfirst($v)?></a>
			</li>
	<?php }else{
?>
<!-- <li
