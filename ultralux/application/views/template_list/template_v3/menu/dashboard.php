...
<div class="container-fluid nopad">
    <div class="row row_bookings_section">
        ...
        <div class="dsh_in">
            <!-- <h3>Dashboard</h3> -->
            <div class="dropdown" id="timeRangeDropdown">
                <button type="button" class="dropdown-button" aria-haspopup="listbox" aria-expanded="false" id="dropdownToggle">
                    ...
                    <span id="selectedLabel">This Month</span>
                    <svg class="icn" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round" style="margin-left:auto;"><polyline points="6 9 12 15 18 9"/></svg>
                </button>
                <ul class="dropdown-menu" role="listbox" tabindex="-1" aria-labelledby="dropdownToggle" id="dropdownMenu">
                    <li class="dropdown-item selected" role="option" tabindex="0" data-value="1">This Month</li>
                    <li class="dropdown-item" role="option" tabindex="0" data-value="3">3 Months</li>
                    <li class="dropdown-item" role="option" tabindex="0" data-value="6">6 Months</li>
                </ul>
            </div>
        ...
        <div class="things_do">
            <div class="container-fluid">
                ...
                <div class="col-xs-12 nopad">
                    ...
                    <div class="thngs_in">
                        <ul>
                            <?php foreach($things_to_do as $ttdo){ ?>
                            <li class="t_water"><span><?= $ttdo['ttdo'] ?></span></li>
                             <?php } ?>
                        </ul>
                    </div>
                </div>
...
<?= (isset($group_time_line_report[5]) ? json_encode($group_time_line_report[5]) . ',' : ''); ?>
 ...
 <script>
    (() => {
      ...
      function fetchDataForRange(months) {
        // Construct URL for current page with query param ?months=X
        const currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set('months', months);

        console.log(`Fetching data for last ${months} month(s) from: ${currentUrl.href}`);

        fetch(currentUrl.href, { method: 'GET' })
          .then(response => {
            if (!response.ok) throw new Error('Network response was not ok');
            return response.text(); // Assuming same page returns HTML/text
          })
          .then(data => {
            // For demo, simply log success and alert user
            console.log('Data fetched successfully.');
            alert(`Data for last ${months} month(s) loaded successfully.`);
            // You can process returned data further here as needed
          })
          .catch(error => {
