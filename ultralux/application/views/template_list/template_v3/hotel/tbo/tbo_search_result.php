...
<script type="text/javascript">
	...
	function calculateNights(checkin, checkout) {
        ...
        // Return the number of nights (should be a positive integer)
        return daysDifference > 0 ? daysDifference : 0; // Ensure no negative nights
    }



        var checkinDate = $('#hotel_checkin').val();
        var checkoutDate = $('#hotel_checkout').val();
 
         var checkin = new Date(checkinDate.split('-').reverse().join('-')); // Convert to YYYY-MM-DD format
       console.log(checkin);
         var checkout = new Date(checkoutDate.split('-').reverse().join('-')); // Convert to YYYY-MM-DD format
      console.log(checkout);
         // Calculate the number of nights
        var numberOfNights = calculateNights(checkin, checkout);
       
        
        $(".nghts").text(numberOfNights+' Nights');
