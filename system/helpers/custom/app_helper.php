 ...
 * Check if current module is enabled
 */
function is_active_crusie_module() {
    $status = false;
 ...
 * Check if current module is enabled
 */
function is_active_air_charter_module() {
    $status = false;
    if (in_array(META_AIRCHARTER_COURSE, $GLOBALS['CI']->active_domain_modules)) {
        $status = true;
    }
    return $status;
}
...
function load_flight_lib($source) {
    ...
    switch ($source) {
      ...
      case PROVAB_FLIGHT_CRS_BOOKING_SOURCE:
            $CI->load->library('flight/flight_crs', '', 'flight_lib');
            break;
        default : redirect(base_url());
    }
}
 ...
 * the given size
 */
function numeric_dropdown_age($size = array('size' => 10)) {
  $options = array();
    if (valid_array($size)) {
        $arr_size = $size ['size'];
        if (isset($size['divider']) == true && $size['divider'] > 0) {
    ...
    switch ($arrangement) {
       ...
       /* Sightseeing */
        ...
        case META_PACKAGE_COURSE : $arrangement_icon = 'fa fa-suitcase';
            break;
		 case META_CRUISE_COURSE: $arrangement_icon = 'fa fa-ship';
            break;
		 case META_AIRCHARTER_COURSE: $arrangement_icon = 'fa fa-plane';
            break;
