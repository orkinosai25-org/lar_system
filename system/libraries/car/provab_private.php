...
class Provab_private extends Common_Api_Grind {
    ...
    private function set_api_credentials(): void
    {
        ...
        $this->system = "test";
        $this->UserName = "test22926789";
        $this->Password = "test@2298@5";
        $this->Url = "http://test.services.travelomatix.com/webservices/index.php/car/service/";
        $this->ClientId = $this->CI->config->item('domain_key');
    }


    private function credentials(string $service): void
    {
        switch ($service) {
            case 'Search':
     ...
     */
    function get_car_list(int $search_id = 0): array
    {
        ...
        if (!$cache_search || ($cache_search && empty($cache_contents))) {
            ...
            $search_response = $this->CI->api_interface->get_json_response(
                $search_request['data']['service_url'],
                $search_request['data']['request'],
                $header_info
            );
     ...
     */
    function car_search_request(array $request_data): array
    {
        $request_params = [
            'pickup_location'           => $request_data['pickup_location'],
            'return_location'             => $request_data['return_location'],
            'pickup_loc_id'      => $request_data['pickup_loc_code'],
            'return_loc_id'        => $request_data['return_loc_code'],
            'pickup_datetime'    => $request_data['pickup_datetime'],
            'return_datetime'   => $request_data['return_datetime'],
            'driver_age'     => $request_data['driver_age'],
        ...
        return [
            ...
            'data' => [
                ...
                'request' => json_encode($request_params, JSON_THROW_ON_ERROR)
            ]
        ];
    }
     ...
     */
    function update_markup_currency(array &$price_summary, object &$currency_obj, float $multiplier = 1.0, bool $level_one_markup = false, bool $current_domain_markup = true): array
    {
        $markup_list = ['EstimatedTotalAmount'];
        $markup_summary = [];
        $book_total_fare = 0;

        foreach ($price_summary as $__k => $__v) {
            if (is_numeric($__v)) {
                // Force currency conversion (by reference)
                $ref_cur = $currency_obj->force_currency_conversion($__v);
                $price_summary[$__k] = $ref_cur['default_value'];

                if (in_array($__k, $markup_list, true)) {
                    $book_total_fare = $__v;
        ...
        // Initialize markup fields
        $price_summary['_Markup'] = 0;
        $markup_summary['_Markup'] = 0;
        $markup_summary['admin_markup'] = '0.00';
        $markup_summary['agent_markup'] = '0.00';
        ...
        if (isset($markup_summary['EstimatedTotalAmount'])) {
            ...
            $markup_summary['OneWayFee'] = is_numeric($markup_summary['OneWayFee'] ?? null) ? round($markup_summary['OneWayFee']) : 0;
            $markup_summary['_Markup'] = $Markup;

            $deduction_cur_obj = clone $currency_obj;
            $markup_total_fare = $currency_obj->get_currency($book_total_fare, true, true, true, $multiplier);
            $ded_total_fare = $deduction_cur_obj->get_currency($book_total_fare, true, false, true, $multiplier);

            $markup_summary['admin_markup'] = sprintf("%.2f", $markup_total_fare['default_value'] - $ded_total_fare['default_value']);
            $markup_summary['agent_markup'] = sprintf("%.2f", $ded_total_fare['default_value'] - $book_total_fare);
        }
     ...
     * update markup currency and return summary
     */
    function update_cancel_markup_currency(array &$price_summary, object &$currency_obj, float $multiplier = 1.0, bool $level_one_markup = false, bool $current_domain_markup = true): array
    {
        $markup_list = ['Amount'];
        $markup_summary = [];
        ...
        foreach ($price_summary as $__k => $__v) {
            if (is_numeric($__v)) {
                $ref_cur = $currency_obj->force_currency_conversion($__v); // Value passed by reference
                $price_summary[$__k] = $ref_cur['default_value'];
        ...
        if (isset($markup_summary['Amount'])) {
            ...
            $markup_summary['ToDate'] = $price_summary['ToDate'] ?? null;
            $markup_summary['CurrencyCode'] = $price_summary['CurrencyCode'] ?? null;
            $markup_summary['FromDate'] = $price_summary['FromDate'] ?? null;
        }
     ...
     */
    public function process_booking(int|string $book_id, array $booking_params): array
    {
        $response = [
            'data' => [],
        ...
        $booking_request = $this->BookCar_request(
            $booking_params,
            $booking_params['booking_source'],
            $book_id
        );
        ...
        $car_book_response = $GLOBALS['CI']->api_interface->get_json_response(
            $booking_request['data']['service_url'],
            $booking_request['data']['request'],
            $header
        );

        $GLOBALS['CI']->custom_db->generate_static_response(json_encode($car_book_response));

        $GLOBALS['CI']->private_management_model->provab_xml_logger(
            'Book_Car',
            $book_id,
            'car',
            json_encode($booking_request['data']),
            json_encode($car_book_response)
        );
        //$car_book_response = $GLOBALS['CI']->hotel_model->get_static_response(286);

        if (valid_array($car_book_response) && ($car_book_response['Status'] ?? false) == true) {
     ...
     * Booking params
     */
    public function BookCar_request(array $booking_params, string $booking_source, int|string $booking_id): array
    {
        ...
        $passenger = [
            ...
            'City'         => $booking_params['city_name'],
            'AddressLine1' => $booking_params['address'],
            'PinCode'      => $booking_params['postal_code'],
            'Email'        => $booking_params['billing_email'],
            'ContactNo'    => $booking_params['passenger_contact'],
            'Title'        => $booking_params['pax_title']
        ];

        // Format Date of Birth
        $dob_parts = explode('-', $booking_params['date_of_birth']);
        $passenger['DateOfBirth'] = "{$dob_parts[2]}-{$dob_parts[1]}-{$dob_parts[0]}";

        // Country details
        $country_data = $this->CI->custom_db->single_table_records(
            'api_country_list',
            'name, iso_country_code',
            ['origin' => (int)$booking_params['country']]
        );
        ...
        if (!empty($booking_params['gps'])) {
            $extra_service['Gps'] = $booking_params['gps'];
        }

        if (!empty($booking_params['add_driver'])) {
            $extra_service['Addtionaldriver'] = $booking_params['add_driver'];
        }

        if (!empty($booking_params['full_prot'])) {
            $extra_service['FullProtection'] = $booking_params['full_prot'];
        }

        if (!empty($booking_params['Infant']) && $booking_params['Infant'] > 0) {
            $extra_service['Infant_equip_count'] = $booking_params['Infant'];
        }

        if (!empty($booking_params['Child']) && $booking_params['Child'] > 0) {
            $extra_service['Child_equip_count'] = $booking_params['Child'];
        }

        if (!empty($booking_params['Booster']) && $booking_params['Booster'] > 0) {
        ...
        $request = [
            'ResultToken'   => $booking_params['token']['ResultToken'],
            'Passengers'    => $passenger,
            'AppReference'  => $booking_id
        ];
	 ...
	 *        	$params
	 */
	function save_booking(string $app_booking_id, array $params, string $module = 'b2c'): array {
        $response = [];
        $attributes = [];
        // echo $module;exit;
		// debug($params);exit;
        ...
        $oneway_fee = 0;
        if(isset($car_data['OneWayFee']['Amount'])){
       ...
       // echo $car_from_date;exit;
		...
		$attributes['air_condition']        = $car_data['AirConditionInd']        ?? null;
        $attributes['transmission_type']    = $car_data['TransmissionType']       ?? null;
        $attributes['fuel_type']            = $car_data['FuelType']               ?? null;
        $attributes['driver_type']          = $car_data['DriveType']              ?? null;
        $attributes['pass_quantity']        = $car_data['PassengerQuantity']      ?? null;
        $attributes['bagg_quantity']        = $car_data['BaggageQuantity']        ?? null; // duplicate line removed
        $attributes['vendor_car_type']      = $car_data['VendorCarType']          ?? null;
        $attributes['door_count']           = $car_data['DoorCount']              ?? null;
        $attributes['vehicle_class_size']   = $car_data['VehClassSize']           ?? null;
        $attributes['unlimited']            = $car_data['Unlimited']              ?? null;
        $attributes['distanceunit']         = $car_data['DistUnitName']           ?? null;

        $attributes['min_age']              = $car_data['RateRestrictions']['MinimumAge']     ?? null;
        $attributes['max_age']              = $car_data['RateRestrictions']['MaximumAge']     ?? null;
        $attributes['payment_rule']         = $car_data['PaymentRules']['PaymentRule']        ?? null;
        $attributes['payment_type']         = $car_data['PaymentRules']['PaymentType']        ?? null;
        $attributes['term_conditions']      = $car_data['TPA_Extensions']['TermsConditions']  ?? null;
        $attributes['supplier_logo']        = $car_data['TPA_Extensions']['SupplierLogo']     ?? null;
        $pickup_schedule = $car_data['LocationDetails']['PickUpLocation']['OperationSchedules'] ?? [];
        $drop_schedule = $car_data['LocationDetails']['DropLocation']['OperationSchedules'] ?? [];

        $pickup_opening_hours = ($pickup_schedule['Start'] ?? '') . '-' . ($pickup_schedule['End'] ?? '');
        $drop_opening_hours   = ($drop_schedule['Start'] ?? '') . '-' . ($drop_schedule['End'] ?? '');

        $car_pickup_location = $search_data['pickup_location'] ?? '';
        $car_drop_location   = $search_data['return_location'] ?? '';

        $pickup_address_data = $car_data['LocationDetails']['PickUpLocation']['Address'] ?? [];
        $drop_address_data   = $car_data['LocationDetails']['DropLocation']['Address'] ?? [];

        $pick_up_phone = $pick_up_phone ?? ''; // assumed defined elsewhere or set to ''
        $drop_phone    = $drop_phone ?? '';

        $car_pickup_address = 
            ($pickup_address_data['StreetNmbr'] ?? '') . ', ' .
            ($pickup_address_data['CityName'] ?? '') . ', ' .
            ($pickup_address_data['PostalCode'] ?? '') . ', ' .
            ($pickup_address_data['CountryName'] ?? '') . ', ' .
            $pick_up_phone;

        $car_drop_address =
            ($drop_address_data['StreetNmbr'] ?? '') . ', ' .
            ($drop_address_data['CityName'] ?? '') . ', ' .
            ($drop_address_data['PostalCode'] ?? '') . ', ' .
            ($drop_address_data['CountryName'] ?? '') . ', ' .
            $drop_phone;

        $attributes['pickup_opening_hours'] = $pickup_opening_hours;
        $attributes['drop_opening_hours']   = $drop_opening_hours;

        $picture_url = $car_data['PictureURL'] ?? '';
        ...
        $itinerary_data = array(
            ...
            'car_model' => $car_model,
            'car_picture_url' => $picture_url
        );
		$this->CI->car_model->save_booking_itinerary_details ( $itinerary_data, $priced_equip, $priced_coverage, $cancellation_poicy, $attrubutes1, $book_total_fare, $admin_markup, $agent_markup, $status);
		...
		$this->CI->car_model->save_booking_pax_details ( $pax_details_data, $country_code, $country_name, $city, $pincode, $adress1, $adress2, $status);
		// debug($params);exit;
        ...
        if ($module == 'b2c') {
            ...
            if(isset($params  ['promo_code_discount_val'])){
		...
		$response ['admin_markup'] = (float)$admin_markup;
		$response ['agent_markup'] = (float)$agent_markup;
     ...
     * Cancel Booking
     */
    function cancel_booking(array $booking_details): array
    {
        $response = array();
        $header = $this->get_header ();
        $response ['data'] = array ();
     ...
     */
    private function cancel_booking_request_params(string $app_reference): array
    {
        ...
        return [
            'status' => true,
            'data' => [
                'request' => json_encode(['AppReference' => $app_reference]),
                'service_url' => $this->service_url
            ]
        ];
    }
    ...
    public function search_data(int $search_id): array
    {
        ...
        if (empty($this->master_search_data) && !valid_array($this->master_search_data)) {
            ...
            if (!empty($clean_search_details['status']) && $clean_search_details['status'] == true) {
                $data = $clean_search_details['data'];

                $data['pickup_location'] = $data['car_from'] ?? '';
                $data['return_location'] = $data['car_to'] ?? '';
                $data['pickup_loc_code'] = $data['car_from_loc_code'] ?? '';
                $data['return_loc_code'] = $data['car_to_loc_code'] ?? '';
                $data['pickup_datetime'] = ($data['depature'] ?? '') . ' ' . ($data['depature_time'] ?? '');
                $data['return_datetime'] = ($data['return'] ?? '') . ' ' . ($data['return_time'] ?? '');
     ...
     */
    public function booking_url(int $search_id): string
    {
     ...
     */
    public function search_data_in_preferred_currency(array $search_result, object $currency_obj): mixed
    {
        $cars = $search_result['data']['CarSearchResult']['CarResults'];
        $car_list = [];

        foreach ($cars as $key => $car) {
            $car['TotalCharge'] = $this->preferred_currency_fare_object($car['TotalCharge'], $currency_obj);
            $car['CancellationPolicy'] = $this->preferred_currency_fare_object_cancel($car['CancellationPolicy'], $currency_obj);
            $car['PricedCoverage'] = $this->preferred_currency_fare_object_coverage($car['PricedCoverage'], $currency_obj);
            $car_list[$key] = $car;
        }
    ...
    public function car_rule_in_preferred_currency(array $rate_rule_result,
    string $module = 'b2c',
    object $currency_obj,
    int $search_id
): array {
     ...
     */
    public function format_search_response(array $hl, object $cobj, int $sid, string $module = 'b2c', array $fltr = []): array {
        // debug($fltr);exit;
        $level_one = true;
        $current_domain = true;
        if ($module == 'b2c') {
           ...
           $ac_var = ($cd['AirConditionInd']=='true')? 'AC': 'Non AC';
            if(
                ...
                (valid_array($fltr['_vehicle_Category']) == false ||
                    (valid_array($fltr['_vehicle_Category']) == true && in_array(strtolower($cd['VehicleCategoryName']), $fltr['_vehicle_Category']))
                    )&&
                (valid_array($fltr['_vehicle_Size']) == false ||
                    (valid_array($fltr['_vehicle_Size']) == true && in_array(strtolower($cd['VehClassSizeName']), $fltr['_vehicle_Size']))
                    )&&
     ...
     * @param unknown_type $currency_obj            
     */
    private function preferred_currency_fare_object(array $fare_details, object $currency_obj, string $default_currency = ''): mixed
    {
        $currency = $default_currency != '' ? $default_currency : get_application_currency_preference();
        $fare_details['CurrencyCode'] = $currency;

        $fare_details['EstimatedTotalAmount'] = round(
            get_converted_currency_value(
                $currency_obj->force_currency_conversion($fare_details['EstimatedTotalAmount'])
            )
        );

        $fare_details['OneWayFee'] = round(
            get_converted_currency_value(
                $currency_obj->force_currency_conversion((int)$fare_details['OneWayFee'])
            )
        );
     ...
     * @param unknown_type $currency_obj            
     */
    private function preferred_currency_fare_object_cancel(array $fare_details, object $currency_obj, string $default_currency = ''): array
    {
        $currency = $default_currency != '' ? $default_currency : get_application_currency_preference();
        $price_details = [];

        foreach ($fare_details as $f_val) {
            $f_val['Amount'] = round(
                get_converted_currency_value(
                    $currency_obj->force_currency_conversion($f_val['Amount'])
                )
            );
            $f_val['CurrencyCode'] = $currency;
            $price_details[] = $f_val;
        }
     ...
     * @param unknown_type $currency_obj            
     */
    private function preferred_currency_fare_object_coverage(array $fare_details, object $currency_obj, string $default_currency = ''): mixed
    {
        $currency = $default_currency != '' ? $default_currency : get_application_currency_preference();

        foreach ($fare_details as $key => $fare) {
            $fare_details[$key]['Currency'] = $currency;

            // Convert and round the amount
            $amount = round(get_converted_currency_value($currency_obj->force_currency_conversion($fare['Amount'])));
            ...
            if (isset($description[1])) {
                ...
                $fare_amount = round(get_converted_currency_value($currency_obj->force_currency_conversion($desc_amount[1])));
                $description = 'with excess up to ' . $fare_amount . ' ' . $currency;
            } else {
     ...
     * @param unknown_type $currency_obj            
     */
    private function preferred_currency_fare_object_equip(array $fare_details, object $currency_obj, string $default_currency = ''): mixed
    {
        foreach ($fare_details as $key => $fare) {
            $currency = empty($default_currency) ? get_application_currency_preference() : $default_currency;
            $fare_details[$key]['CurrencyCode'] = $currency;

            $amount = get_converted_currency_value($currency_obj->force_currency_conversion($fare['Amount']));
            $fare_details[$key]['Amount'] = round($amount);
        }
        return $fare_details;
    }
     ...
     */
    function update_search_markup_currency(array &$price_summary, object &$currency_obj, int $search_id, bool $level_one_markup = false, bool $current_domain_markup = true): array
    {
        $multiplier = 1;
     ...
     * @param number $search_id         
     */
    function update_cancellation_markup_currency(array &$price_summary, object &$currency_obj, int $search_id, bool $level_one_markup = false, bool $current_domain_markup = true): array
    {
        $cancellation_policy = [];

        foreach ($price_summary as $p_key => $price) {
            $cancellation_policy[$p_key] = $this->update_cancel_markup_currency($price, $currency_obj, 1, $level_one_markup, $current_domain_markup);
        }
     ...
     * @param array $hl
     */
    function filter_summary(array $hl): array{
        $h_count = 0;
        $filt = array();
        $filt ['p'] ['max'] = false;
        $filt ['p'] ['min'] = false;
        ...
        foreach ( $hl ['CarSearchResult'] ['CarResults'] as $hr => $hd ) { 
            ...
            if(isset($hd) && valid_array($hd)) {
                ...
                $car_type = $hd['VehicleCategoryName'];
                ...
                if(isset($size_list) && !empty($size_list)) {
                    ...
                    if(isset($filt['vehicle_size'][$vehicle_size]) == false) {
                        ...
                        $filt ['vehicle_size'] [$vehicle_size] ['y'] = $vehicle_size;
                    }else {
	 ...
	 */
    function get_rate_rules(string $ResultIndex): array
    {
        $header = $this->get_header();
        ...
        if ($car_rule_request['status']) {
            // Fetching car rule response
            $car_rule_response = $GLOBALS['CI']->api_interface->get_json_response(
                $car_rule_request['data']['service_url'],
                $car_rule_request['data']['request'],
                $header
            );
	 ...
	 *  $hotel_details
	 */
    private function valid_car_rules(array $car_rules): bool
    {
        return valid_array($car_rules) && isset($car_rules['Status']) && $car_rules['Status'] == SUCCESS_STATUS;
    }
	/**
	 * Anitha G
	 *
	 ...
	 *    	
	 */
    private function car_rule_request(string $ResultToken): array
    {
     ...
     */
    public function convert_token_to_application_currency(array $token, $currency_obj, string $module): array
    {
        $token['TotalCharge']['EstimatedTotalAmount'] = get_converted_currency_value(
            $currency_obj->force_currency_conversion($token['TotalCharge']['EstimatedTotalAmount'])
        );

        $master_token = $token;
        $seat_attr = $token['seat_attr'];
        $seat_attr['markup_price_summary'] = get_converted_currency_value(
            $currency_obj->force_currency_conversion($seat_attr['markup_price_summary'])
        );
        $seat_attr['total_price_summary'] = get_converted_currency_value(
            $currency_obj->force_currency_conversion($seat_attr['total_price_summary'])
        );
        $seat_attr['domain_deduction_fare'] = get_converted_currency_value(
            $currency_obj->force_currency_conversion($seat_attr['domain_deduction_fare'])
        );
        $seat_attr['default_currency'] = admin_base_currency();

        // Converting seats
        foreach ($seat_attr['seats'] as $sk => $sv) {
            $seat_attr['seats'][$sk] = $sv;
            $seat_attr['seats'][$sk]['Fare'] = get_converted_currency_value(
                $currency_obj->force_currency_conversion($sv['Fare'])
            );
            $seat_attr['seats'][$sk]['Markup_Fare'] = get_converted_currency_value(
                $currency_obj->force_currency_conversion($sv['Markup_Fare'])
            );
        }
     ...
     *          $limit
     */
    function get_page_data(array $hl, int $offset, int $limit): array
    {
     ...
     */
    function valid_api_response(array $api_response): bool
    {
        return !empty($api_response) && valid_array($api_response) && isset($api_response['Status']) && $api_response['Status'] == SUCCESS_STATUS;
    }
}
