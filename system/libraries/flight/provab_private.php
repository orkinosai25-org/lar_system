...
class Provab_private extends Common_Api_Grind {
    ...
    function calendar_fare_request($search_params) {
		...
		if(isset($search_params['return']) && empty($search_params['return']) == false){
			$segments['ReturnDate'] = $search_params['return'];
		}
    ...
    function format_cheap_fare_list($raw_fare_list, $strict_format = false) {
        ...
        //debug($GetCalendarFareResult);exit;
        if (valid_array($GetCalendarFareResult) == true) {
            ...
            foreach ($LowestFareOfDayInMonth as $k => $day_fare) {
                if (valid_array($day_fare) == true) {
                   // debug($day_fare);exit;
                    $depature_date = explode(" ", $day_fare['DepartureDate']);
                    ...
                    $fare_list_obj['departure_date'] = date('D d', strtotime($depature_date[0]));
                    $fare_list_obj['departure_time'] = date('H:i', strtotime($depature_date[1]));
					...
					$fare_list_obj['date'] = $day_fare['Date'];
                    //debug($fare_list_obj);exit;
                } else {
                    $fare_list_obj = false;
                }
                if (valid_array($day_fare) == true) {
                    $fare_list[$day_fare['DepartureDate']] = $fare_list_obj;
                }
            }
            $response['data'] = $fare_list;
        } else {
...
//debug($pax_ticket_ids);exit;
        ...
        for ($i = 0; $i < $total_pax_count; $i++) {
            ...
            if (empty($passenger['passport_number'][$i]) == false and empty($passenger['passport_expiry_date'][$i]) == false) {
                ...
                $tmp_passenger[$i]['PassportExpiry'] = $passenger['passport_expiry_date'][$i];
			 	$tmp_passenger[$i]['PassporIssuingCountry'] = $passenger['passenger_passport_issuing_country'][$i];
				
            } else {
                $tmp_passenger[$i]['PassportNumber'] = '';
                $tmp_passenger[$i]['PassportExpiry'] = null;
				$tmp_passenger[$i]['PassporIssuingCountry'] = null;
            }
    ...
    private function extract_passenger_info($booking_params, $SequenceNumber) {
        ...
        foreach ($booking_params['passenger_type'] as $__k => $__v) {
			...
			$gender = (isset($__v) ? get_enum_list('gender', $booking_params['gender'.($__k+1)]) : '');
    ...
    function calendar_safe_search_data($search_data) {
		...
		if (isset($search_data['return']) == true and empty($search_data['return']) == false) {
            if (strtotime($search_data['return']) < time()) {
                $safe_data['return'] = date('Y-m-d');
            } else {
                $safe_data['return'] = date('Y-m-d', strtotime($search_data['return']));
            }
        } else {
            $safe_data['return'] = '';
        }
		//debug($safe_data);exit;
		if($search_data['trip_type'] == 'circle'){
        //Type
        	$safe_data['trip_type'] = 'return';
		}
		else{
    ...
    function update_markup_currency(& $price_summary, & $currency_obj, $level_one_markup = false, $current_domain_markup = true, $multiplier = 1, $specific_markup_config = array(), $module='') {
        ...
        if (isset($markup_summary['OfferedFare'])) {
            ...
            if($Markup < 2){
                $Markup = 0;
                $markup_summary['OfferedFare'] =  $price_summary['OfferedFare'];
            }
        ...
        if($module == 'b2c'){
            if($Markup > 0 ){
				$gst_details = $GLOBALS['CI']->flight_model->get_gst_details();
    ...
    function total_price($price_summary, $retain_commission = false, $currency_obj = '') {
        ...
        $gst = 0;
        if(isset($price_summary['_GST'])){
            $gst = $price_summary['_GST'];
        }
        return (floatval(@$price_summary['OfferedFare']) + $com + $com_tds+$gst);
    }
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $total_price['_AgentMarkup'] = $total_price['_Markup'] = $agent_price_details['_Markup'] + $admin_price_details['_Markup'];
...
//debug($extra_services['Seat']);exit;
            foreach ($extra_services['Seat'] as $seat_k => $seat_v) {
                foreach ($seat_v as $sd_k => $sd_v) {
                    if($sd_k!="SeatColumn" && $sd_k!="Description"){
                    foreach ($sd_v as $seat_index1 => $seat_value1) {
                       // debug($seat_value1);
						 foreach ($seat_value1 as $seat_index => $seat_value) {
							//Convert the Price to prefeerd currency
							$Price = $seat_value['Price'];
							$Price = get_converted_currency_value($currency_obj->force_currency_conversion($Price));
							$extra_services['Seat'][$seat_k][$sd_k][$seat_index1][$seat_index]['Price'] = $Price;
							$extra_services['Seat'][$seat_k][$sd_k][$seat_index1][$seat_index]['JourneyType'] = $journey_type;
						 }
                    }
                }
            }
    ...
    public function convert_extra_services_to_application_currency($extra_service_details, $currency_obj) {
      ...
      //debug($extra_service_details['data']['ExtraServiceDetails']['Seat']);exit;
        if (isset($extra_service_details['data']['ExtraServiceDetails']['Seat']) == true && valid_array($extra_service_details['data']['ExtraServiceDetails']['Seat']) == true) {
            ...
            foreach ($extra_service_details['data']['ExtraServiceDetails']['Seat'] as $Seat_k => $Seat_v) {
                ...
                foreach ($Seat_v as $st_k => $st_v) {
                     ...
                     if($st_k!="SeatColumn" && $st_k!="Description"){
                   foreach($st_v as $seat_ind_k1 => $seat_ind_v1) {
                    foreach($seat_ind_v1 as $seat_ind_k => $seat_ind_v) {
                      // debug($seat_ind_v);
                    //Convert the Price to application currency
                    $Price = $seat_ind_v['Price'];
                    $Price = get_converted_currency_value($currency_obj->force_currency_conversion($Price));
                    $extra_service_details['data']['ExtraServiceDetails']['Seat'][$Seat_k][$st_k][$seat_ind_k1][$seat_ind_k]['Price'] = $Price;
                    }
                }
                }
            }
    ...
    public function preferred_currency_paxwise_breakup_object($fare_details, $currency_obj) {
        ...
        foreach ($fare_details as $k => $v) {
            ...
            if (isset($v['BasePrice'])) {
			 ...
			 if (isset($v['Tax'])) {
                $tax = $PassengerFareBreakdown[$k]['Tax'];
                unset($PassengerFareBreakdown[$k]['Tax']);
            } else {
                $tax = $PassengerFareBreakdown[$k]['Tax'];
            }
             if (isset($v['TotalPrice'])) {
                $totalprice = $PassengerFareBreakdown[$k]['TotalPrice'];
                unset($PassengerFareBreakdown[$k]['TotalPrice']);
            } else {
                $totalprice = $PassengerFareBreakdown[$k]['TotalPrice'];
            }
            $PassengerFareBreakdown[$k]['BaseFare'] = get_converted_currency_value($currency_obj->force_currency_conversion($base_fare));
            $PassengerFareBreakdown[$k]['Tax'] = get_converted_currency_value($currency_obj->force_currency_conversion($tax));
            $PassengerFareBreakdown[$k]['TotalPrice'] = get_converted_currency_value($currency_obj->force_currency_conversion($totalprice));
        }
        return $PassengerFareBreakdown;
    }
    ...
    public function extract_segment_details($segment_details) {
        ...
        foreach ($segment_details as $seg_k => $seg_v) {
            ...
            foreach ($seg_v as $seg_details_k => $seg_details_v) {
                ...
                $segment_full_details[$seg_k][$seg_details_k]['StopOver'] = '';

                if(isset($seg_details_v['stop_over'])){
  ...
  //       echo "<br/>";
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
        ...
        foreach ($token as $token_index => $token_value) {
            ...
            $WSTicket = array();
            $WSFareRule = array();
            $ticket_trans_status_group = array();
            ...
            $transaction_det = array(
                ...
                'source' => $source,
                'ref_id' => $ref_id,
                'attributes' => json_encode($tranaction_attributes),
                'sequence_number' => $sequence_number,
            );
            $transaction_price = array(
                'currency' => $currency,
                'commissionable_fare' => $commissionable_fare,
            ...
            $transaction_insert_id = $GLOBALS['CI']->flight_model->save_flight_booking_transaction_details(
                    $transaction_det, $transaction_price
            );
            ...
            for ($i = 0; $i < $total_pax_count; $i++) {
                ...
                $gender = get_enum_list('gender', $book_params['gender'.($i+1)]);
                ...
                $pax_insert_id = $GLOBALS['CI']->flight_model->save_flight_booking_passenger_details(
                        $passenger_det, $passenger_nationality, $passport_number, $passport_issuing_country, $passport_expiry_date, $status, json_encode($passenger_attributes), $flight_booking_transaction_details_fk);
            ...
            foreach ($segment_details as $seg_k => $seg_v) {
                ...
                foreach ($seg_v as $ws_key => $ws_val) {
                    ...
                    $GLOBALS['CI']->flight_model->save_flight_booking_itinerary_details(
                            $flight_itinerary_det, json_encode($attributes), $FareRestriction, $FareBasisCode, $FareRuleDetail, $airline_pnr, $cabin_baggage, $checkin_baggage, $is_refundable);
                }
        ...
        $last_segment_group = end($segment_summary);       // Get the last group (e.g., last leg)
        $journey_end = end($last_segment_group);           // Then get the last segment in that group
        ...
        $flight_booking_data= array(
				'app_reference' => $app_reference,
				'sequence_number' => $sequence_number,
				'domain_origin' => $domain_origin,
				'flight_booking_status' => $flight_booking_status,
				...
				'booking_details_attributes' => json_encode($attributes),
			);
        $GLOBALS['CI']->flight_model->save_flight_booking_details(
                $flight_booking_data, $created_by_id, $from_loc, $to_loc, $from_to_trip_type, $transaction_currency, $currency_conversion_rate, $gst_details, $phone_country_code);
                ...
                if ($this->valid_flight_booking_status($ticket_value['Status']) == true) {//IF Ticket is HOLD/CONFIRMED
                   ...
                   // $update_transaction_data['agent_markup'] = $agent_markup;
                    ...
                    foreach ($passenger_details as $pax_k => $pax_v) {
                        ...
                        $IssueDate = '';
                        $Fare = (float)json_encode($single_pax_fare_breakup[$pax_v['PassengerType']]);
                        ...
                        $GLOBALS['CI']->flight_model->update_passenger_ticket_info($passenger_update_data, $CorporateCode, $TourCode, $Endorsement, $Remarks, $ServiceFeeDisplayType, $api_passenger_origin);
                    }
            ...
            if (isset($ExtraServiceDetails['Seat']) == true && valid_array($ExtraServiceDetails['Seat']) == true) {
                ...
                foreach ($Seat as $os_k => $os_v) {
                    foreach ($os_v as $sk => $sv) {
                        if($sk!="SeatColumn" && $sk!="Description"){
                        foreach ($sv as $seat_index1 => $seat_value1) {
							foreach ($seat_value1 as $seat_index => $seat_value) {
