...
class Common_Flight {
	 ...
	 */
	static function insert_record(string $key, mixed $value): array {
		$ci = & get_instance ();
	...
	/**
	 */
	static function read_record(string $key, int $offset = -1, int $limit = -1): mixed {
		$ci = & get_instance ();
	 ...
	 * @return array[]
	 */
	static function insert_string(string $key, string $value): void {
		$ci = & get_instance ();
	 ...
	 * @param number $limit        	
	 */
	static function read_string(string $key): string {
		$ci = & get_instance ();
	 ...
	 *        	$booking_source
	 */
	public function validate_temp_booking_id(string $app_reference,
    string $token,
	 ...
	 * save flight details
	 */
	function save_flight_booking(array $flight_data,
	...
	): array
	{
		...
		if($porceed_to_save['status'] != SUCCESS_STATUS){
			...
			$ci->flight_model->save_flight_booking_details ( $domain_origin, $flight_booking_status, $app_reference, $booking_source, $is_lcc, $currency, $phone, $alternate_number, $email, $journey_start, $journey_end, $journey_from, $journey_to, $payment_mode, json_encode($booking_details_attributes), $created_by_id,$currency_conversion_rate, FLIGHT_VERSION_2);
	 ...
	 * @param string $flight_booking_status
	 * @param string $app_reference
	...
	public function update_flight_booking_status(string $flight_booking_status, string $app_reference, int $sequence_number, string $booking_source): void
	{
		...
		$flight_booking_transaction_details_condition = array(); 
	 ...
	 */
	public function update_flight_booking_tranaction_price_details(string $app_reference,
    int $sequence_number,
	 ...
	 */
	public function update_passenger_ticket_info(int $passenger_fk,
    string $ticket_id,
    string $ticket_number,
    array $pax_break_down = []
	): void
	{
	...
	public function deduct_flight_booking_amount(string $app_reference, int $sequence_number): void
	{
		...
		$data = $ci->db->query(
        'SELECT BD.currency, BD.currency_conversion_rate, FT.* 
         FROM flight_booking_details BD
         JOIN flight_booking_transaction_details FT ON BD.app_reference = FT.app_reference
         WHERE FT.app_reference = ? AND FT.sequence_number = ?',
        [$app_reference, $sequence_number]
    )->row_array();
	...
	public function deduct_flight_booking_amount_hold($app_reference, $sequence_number)
	{
		...
		$condition['sequence_number'] = $sequence_number;
		$data = $ci->db->query(
			'select BD.currency,BD.domain_origin,BD.currency_conversion_rate,FT.* from flight_booking_details BD
						join flight_booking_transaction_details FT on BD.app_reference=FT.app_reference
						where FT.app_reference="' . trim($app_reference) . '" and FT.sequence_number=' . $sequence_number
		)->row_array();

		if (valid_array($data) == true && in_array($data['status'], array('BOOKING_CONFIRMED')) == true) { //Balance Deduction for Hold Booking when Admin upload the PNR and Ticket
			$ci->load->library('booking_data_formatter');
			$transaction_details = $data;
			$agent_buying_price = $ci->booking_data_formatter->agent_buying_price($transaction_details);
			...
			$ci->domain_management->debit_domain_balance_hold_booking($agent_buying_price, 'live', $data['domain_origin'], $domain_booking_attr); //deduct the domain balance
			//Save to Transaction Log
			$domain_markup = $transaction_details['domain_markup'];
			$level_one_markup = 0;
			$agent_transaction_amount = $agent_buying_price - $domain_markup;
	 ...
	 * @param array $data        	
	 */
	function save_ssr_details(string $app_reference, array $data): void{
	 ...
	 */
	public function update_markup_and_insert_cache_key_to_token(array $flight_list, string $carry_cache_key, string $search_id): array
	{
			// debug($carry_cache_key);
	...
	public function cache_extra_services(array $extra_services, string $carry_cache_key): array
	{
			...
			if(isset($extra_services['Seat']) == true && valid_array($extra_services['Seat']) == true){
				foreach ($extra_services['Seat'] as $seat_k => $seat_v){
					foreach ($seat_v as $sd_k => $sd_v){
						$Seg_Info = $sd_v['Seat_Info'];
						foreach($Seg_Info as $seat_index => $seat_value){
							$Seat_Info_Details = $seat_value['Seat_Info'];
							foreach ($Seat_Info_Details as $s_key => $s_value) {
								$access_data = Common_Flight::insert_record ( $carry_cache_key, json_encode ( $s_value ) );
								$extra_services['Seat'][$seat_k][$sd_k]['Seat_Info'][$seat_index]['Seat_Info'][$s_key]['SeatId'] = $access_data ['access_key'];
							
								//Convert the Price to Domain Currency
								$extra_services['Seat'][$seat_k][$sd_k]['Seat_Info'][$seat_index]['Seat_Info'][$s_key]['Price'] = get_converted_currency_value($currency_obj->force_currency_conversion($seat_value['Price']));
							}
						}
					}
				}
		...
		if($_SERVER['REMOTE_ADDR'] =='192.168.0.60'){
			...
			if(isset($extra_services['MealPreference']) == true && valid_array($extra_services['MealPreference']) == true){
				foreach ($extra_services['MealPreference'] as $mp_key => $mp_value) {
					foreach ($mp_value as $meal_k => $meal_v){
						$meal_list = $meal_v['Meal_Info'];
						foreach ($meal_list as $md_k => $md_v){
							$access_data = Common_Flight::insert_record ( $carry_cache_key, json_encode ( $md_v ) );
							$extra_services['MealPreference'][$mp_key][$meal_k]['Meal_Info'][$md_k]['MealId'] = $access_data ['access_key'];
						}
					}
				}
	 ...
	 */
	private function update_fare_markup_commission(array &$FareDetails,
    float $multiplier,
	...
	private function convert_to_domain_currency_object(array &$FareDetails, bool $domain_currency_conversion = true): void
	{
		...
		$currency_obj = new Currency(array('from' => 'USD' , 'to' => $domain_base_currency));
	 ...
	 */
	public function final_booking_transaction_fare_details( array $core_price_details, 
    int $search_id, 
    string $booking_source, 
    string $OperatorCode = ''
	): array
	{
		$ci = & get_instance();
		$final_booking_transaction_fare_details = array();
		$multiplier = $this->get_markup_multiplier($search_id);
	...
	public function get_flight_booking_transaction_details(string $app_reference, 
    int $sequence_number, 
    string $booking_source = '', 
	 ...
	 */
	private function format_flight_booking_details(array $flight_booking_details): array
	{
		$ci = & get_instance();
	 ...
	 */
	function update_calendarfare_currency(array $FareDetails): array
	{
	 ...
	 */
	private function update_agent_commision(float $amount, float $commission_percentage): float
	{
	 ...
	 */
	private function get_markup_multiplier(int $search_id): int
	{
		$ci = & get_instance ();
		$search_data = $ci->flight_model->get_safe_search_data ( $search_id );
		$search_data = $search_data ['data'];
	...
	function get_airline_list(): array {
		$CI = & get_instance ();
	 ...
	 *        	$class_name
	 */
	function get_airline_class_label(string $class_name): string {
		$label = '';
		if (isset ( $class_name )) {
			switch ($class_name) {
				case 'C' :
	 ...
	 */
	static function get_round_price(float|int $price): float|int {
	 ...
	 */
	function filter_booking_source(array $active_booking_source_list, array $safe_search_data): array {
		$active_source = array ();
	 ...
	 * @param array $passenger_fare_breakdown
	 */
	public function get_single_pax_fare_breakup(array $passenger_fare_breakdown): array
	{
	 ...
	 * @param string $app_reference
	 * @param int|string $sequence_number
	 * @param bool $is_full_booking_cancel
	 * @param string $booking_source
	 */
	public function elgible_for_ticket_cancellation(string $app_reference, int|string $sequence_number, array $ticket_ids, bool $is_full_booking_cancel, string $booking_source): array
	{
		$ci = & get_instance ();
		$response  = array();
		$response ['status'] = FAILURE_STATUS;
			...
			if($travel_date >= time()){
				if($is_full_booking_cancel == true){
					...
					$index_passenger_orign = array();
                                  ...
                                  if($booking_source==TRAVELPORT_FLIGHT_BOOKING_SOURCE)
                                  {
                                      foreach ($booking_customer_details as $pax_k => $pax_v){
						$index_passenger_orign[$pax_v['TicketId']] = $pax_v['status'];
					}
                                  }
                                  else
                                  {
					...
					foreach ($ticket_ids as $k => $v){
						if(isset($index_passenger_orign[$v]) == true && $index_passenger_orign[$v] == 'BOOKING_CONFIRMED'){
							$ticket_status = SUCCESS_STATUS;
						} else {
	 ...
	 * @param string $app_reference
	 * @param int $sequence_number
	 * @param int|string $passenger_origin
	 */
	public function update_ticket_cancel_status(string $app_reference, int|string $sequence_number, int|string $passenger_origin): void
	{
		$ci = & get_instance();
	 ...
	 */
	public function get_cancellation_reequested_pax_details(array $booking_customer_details,
    array $passenger_origins
	): array	
	{
