...
abstract class Common_Api_Flight extends Common_Api_Grind {
	 ...
	 */
	protected function format_summary_array( int|string $journey_number,
    string $origin_code,
	 ...
	 *        	source_code
	 */
	function read_session_id(string $key = ''): ?array {
		if (empty ( $key ) == true) {
			$key = $this->source_code;
		}
	 ...
	 */
	function get_flight_book_record(string $app_reference, array $train_filter = [], array $customer_filter = []): array {
		$ci = & get_instance ();
	 ...
	 * @param string $status        	
	 */
	function update_passenger_record(string $book_id, int $sindex, string $status): void {
		$cond = array();
		$data = array();
