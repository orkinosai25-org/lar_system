...
class Provab_private extends Common_Api_Grind {
	...
	function credentials($service) {
		switch ($service) {
			...
			case 'GetRoomFacilities' :
				$this->service_url = $this->Url . 'RoomFacilities';
				break;
			case 'BlockRoom' :
	...
	private function hotel_search_request($search_params) {
		...
		if(isset($search_params ['hotelcode']) && empty($search_params ['hotelcode']) == false){
			$request ['hotelcode'] = $search_params ['hotelcode'];
		}
	 ...
	 * @param string $HotelCode        	
	 */
	private function room_facilities_request($params) {
		$response ['status'] = true;
		$response ['data'] = array ();
		$request ['hotel_code'] = $params['hotel_code'];
		$request ['room_code'] = $params['room_code'];
		$response ['data'] ['request'] = json_encode ( $request );
		$this->credentials ( 'GetRoomFacilities' );
		$response ['data'] ['service_url'] = $this->service_url;
		return $response;
	}
	...
	*/
	 ...
	 *        	unique id which identifies search details
	 */
	function get_hoteldetails_list($search_id = '') {
		$header = $this->get_header ();
		$response ['data'] = array ();
		$response ['status'] = true;
		$search_data = $this->search_data ( $search_id );
	...
	*/
	 ...
	 * @param string $HotelCode        	
	 */
	function get_room_facilities($params) {
		$header = $this->get_header ();
		$response ['data'] = array ();
		$response ['status'] = false;
		$hotel_room_request = $this->room_facilities_request ($params);
		if ($hotel_room_request ['status']) {
			// get the response for hotel details
			$hotel_room_facilities_response = $GLOBALS ['CI']->api_interface->get_json_response ( $hotel_room_request ['data'] ['service_url'], $hotel_room_request ['data'] ['request'], $header );
			$GLOBALS ['CI']->custom_db->generate_static_response ( json_encode ( $hotel_room_facilities_response ) );
			// debug($hotel_room_list_response);exit;
			/*
			 * $static_search_result_id = 813;//106;//68;//52;
			 * $hotel_room_list_response = $GLOBALS['CI']->hotel_model->get_static_response($static_search_result_id);
			 */
			if ($this->valid_room_details_details ( $hotel_room_facilities_response )) {
				$response ['data'] = $hotel_room_facilities_response['RoomFacilities'];
				$response ['status'] = true;
			} else {
				$response ['data'] = $hotel_room_facilities_response;
			}
		}
		return $response;
	}
	...
	function process_booking($book_id, $booking_params)
	{
		...
		//$book_response = $GLOBALS['CI']->hotel_model->get_static_response(212);
	...
	function save_booking($app_booking_id, $params, $module = 'b2c') {
		...
		$hotel_code = $params['booking_params']['token']['hotel_code'];;
		...
		$city_id = $search_data['data']['CityId'];
		//$check_out_time = '';
		//$check_in_time = '';
		...
		foreach ( $HotelRoomsDetails as $k => $v ) {
			...
			$itinerary_price = array(
				'RoomPrice' => $v ['Price'] ['RoomPrice'],
				'Tax' => $v ['Price'] ['Tax'],
				'ExtraGuestCharge' => $v ['Price'] ['ExtraGuestCharge'],
				'ChildCharge' => $v ['Price'] ['ChildCharge'],
				'OtherCharges' => $v ['Price'] ['OtherCharges'],
				'Discount' => $v ['Price'] ['Discount'],
				'ServiceTax' => $v ['Price'] ['ServiceTax'],
				'AgentCommission' => $v ['Price'] ['AgentCommission'],
				'AgentMarkUp' => $v ['Price'] ['AgentMarkUp'],
				'TDS' => $v['Price']['TDS']
			);
			$GLOBALS ['CI']->hotel_model->save_booking_itinerary_details ($itinerary_data, $itinerary_price, $gst_value );
			...
			if (valid_array ( $passengers ) == true) {
				foreach ( $passengers as $pass_key => $passenger ) {
					...
					$attributes = array ();
					$gender_list = array(1=>'Male', 2=>'Female', 3 => 'non-binary', 4=>'opt out');
					$gender = $gender_list[$params['booking_params']['gender'.($pass_key+1)]];
					
					//debug($params['booking_params']);exit;
					...
					$GLOBALS ['CI']->hotel_model->save_booking_pax_details ( $app_reference, $personal_details, $passenger_nationality, $passport_number, $passport_issuing_country, $passport_expiry_date, $status, serialize ( $attributes ),$gender );
				}
			}
		}
	...
	*/
	function update_cancellation_markup_currency(&$cancel_charge,&$currency_obj,$search_id,$level_one_markup=false,$current_domain_markup=true){
		...
		$gst_value = 0;
		$cancel_charge = $currency_obj->force_currency_conversion ( $cancel_charge );

		$Markup = $temp_price['default_value'] - $cancel_charge['default_value'];
		$gst_value = 0;
        //adding gst
	    if($Markup > 0 ){
			 $gst_details = $GLOBALS['CI']->hotel_model->get_gst_details();
