...
class Master_currency {
    ...
    public function currency_conversion_value($override_conversion_rate = false, $from='', $to='') {
        if ($override_conversion_rate == true || empty($this->conversion_cache[$from . $to]) == true) {
            if ($from != $to) {
                ...
                $get = file_get_contents("https://prod.services.travelomatix.com/webservices/index.php/rest/currecny_value_details?amount=$encode_amount&from=$from_Currency&to=$to_Currency");
                // debug($get);exit;
    ...
    function get_currency($value, $add_markup = true, $parent_markup = false, $seller_markup = true, $markup_multiplier = 1, $specific_markup_config = array()) {
        ...
        return array('default_value' => number_format($value['value'], 2, '.', ''), 'default_currency' => $this->to_currency,'original_markup' => $value['original_markup'],'markup_type' => $value['markup_type']);
    }
