...
class Template {
	...
	public function domain_airline_full_path($image_name='')
	{
		if (empty($image_name) == true || is_string($image_name) == false) {
			return DOMAIN_AIRLINE_UPLOAD_DIR;
		} else 	{
			return DOMAIN_AIRLINE_UPLOAD_DIR.$image_name;
		}
	}
	...
	*/
	...
	/*for uploading the top airline logo*/
	public function domain_hotel_partner_upload_path(){
		return DOMAIN_HOTEL_PARTNER_UPLOAD_DIR;
	}
	/**
	 * get domain specific image dir path details
	 */
	public function domain_hotel_partner_images($image_name='')
	{
		if (empty($image_name) == true || is_string($image_name) == false) {
			return DOMAIN_HOTEL_PARTNER_IMAGE_DIR;
		} else 	{
			return DOMAIN_HOTEL_PARTNER_IMAGE_DIR.$image_name;
		}
	}
	...
	public function domain_view_ecoimage()
	{
		return HOTEL_CRS_VIEW_IMAGES;
	}

	public function domain_eco_stays_images_upload_dir_full_path()
	{
		return realpath('../extras') . '/custom/' . CURRENT_DOMAIN_KEY . '/uploads/eco_stays/';
	}
	public function domain_eco_stays_images_upload_dir($image_name = '')
	{
		//return HOTEL_CRS_IMAGE_UPLOAD_DIR;
		$path = '../' . CUSTOM_RESOURCE_DIR . '/' . CURRENT_DOMAIN_KEY . '/uploads/eco_stays/';
		if (empty($image_name) == true || is_string($image_name) == false) {
			return $path;
		} else {
	...
	public function domain_image_full_path123($image_name='')
	{
		if (empty($image_name) == true || is_string($image_name) == false) {
			return '../..'.DOMAIN_IMAGE_DIR;
		} else 	{
