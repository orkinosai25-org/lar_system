...
class Module extends CI_Controller {
	...
	function module_management(): void
	{
		$map_data=[];
		$insert_map_data=[];
		$page_data=[];
		...
		if (valid_array($page_data['form_data']) == false && isset($page_data['eid']) == true) {
			...
			if ($this->form_validation->run()) {
				...
				if (intval($page_data['form_data']['origin']) > 0) {
					...
					$insert_data['tds'] = 0;
					$insert_data['gst'] = 0;
					$insert_data['meta_course_list_fk'] = $insert_id['insert_id'];
					$insert_data['created_by_id'] = $this->entity_user_id;
					$insert_data['created_date'] = date('Y-m-d H:i:s');
					$insert_data['modified_date'] = date('Y-m-d H:i:s');
					$insert_id = $this->custom_db->insert_record('gst_master', $insert_data);
	...
	function booking_source_management(): void{
		$page_data=[];
		$insert_map_data=[];
		$page_data['form_data'] = $this->input->post();
		$get_data = $this->input->get();
		...
		if (valid_array($page_data['form_data']) == false && isset($page_data['eid']) == true) {
			 ...
			 * EDIT DATA
			 */
			$edit_data = $this->module_model->booking_source_management($page_data['eid'], $page_data['course_id']);
			...
			if ($this->form_validation->run()) {
				...
				if (intval($page_data['form_data']['origin']) > 0) {
					...
					$this->custom_db->update_record('booking_source', $page_data['form_data'], array('origin' => $page_data['eid']));
					/** Update booking_source and activity reference **/
					$meta_course_list_fk = $this->custom_db->single_table_records('meta_course_list','origin', array('course_id' => $page_data['form_data']['meta_course_list_id']));
					$this->custom_db->delete_record('activity_source_map',array('booking_source_fk' => $page_data['eid']));
					/** Inserting booking_source and activity reference **/
					$insert_map_data['meta_course_list_fk']=$meta_course_list_fk['data'][0]['origin'];
					$insert_map_data['domain_origin'] = get_domain_auth_id();
					$insert_map_data['booking_source_fk']=$page_data['eid'];
					...
					$page_data['form_data']['Source_id'] = 'PTBSID000000000'.$random_number;
					//debug($page_data);exit;
					$insert_id = $this->custom_db->insert_record('booking_source', $page_data['form_data']);

					if( $insert_id['status'] == SUCCESS_STATUS ) {
						$meta_course_list_fk = $this->custom_db->single_table_records('meta_course_list','origin', array('course_id' => $page_data['form_data']['meta_course_list_id']));
						/** Inserting booking_source and activity reference **/
						$insert_map_data['domain_origin'] = get_domain_auth_id();
						$insert_map_data['meta_course_list_fk']=$meta_course_list_fk['data'][0]['origin'];
						$insert_map_data['booking_source_fk']=$insert_id['insert_id'];
						$this->custom_db->insert_record('activity_source_map', $insert_map_data);
				...
				redirect('module/booking_source_management');
			}
		}
		/** TABLE PAGINATION */
		$page_data['table_data'] = $this->module_model->get_booking_source_list();
		//debug($page_data['table_data']);exit;
		$this->template->view('module/booking_source_management', $page_data);
	}
}