...
if(is_active_car_module()){
			array_push($monthly_series_data, $this->monthly_car_search_history_log($year_start, $year_end));
			$page_data['car_top_search'] = json_encode($this->car_top_search($year_start, $year_end));
		}
	...
	private function sightseeing_top_search(string $year_start, string $year_end): array{
		$this->load->model('sightseeing_model');
		$temp_data = $this->sightseeing_model->top_search($year_start, $year_end);
		return $this->group_top_search_data($temp_data);

	}
	private function transfer_top_search(string $year_start, string $year_end): array{
		$this->load->model('transferv1_model');
		$temp_data = $this->transferv1_model->top_search($year_start, $year_end);
		return $this->group_top_search_data($temp_data);

	}
	private function car_top_search(string $year_start, string $year_end): array{
		$this->load->model('car_model');
		$temp_data = $this->car_model->top_search($year_start, $year_end);
		return $this->group_top_search_data($temp_data);

	}

	private function bus_top_search(string $year_start, string $year_end): array
	{
	...
	private function monthly_bus_search_history_log(string $year_start, string $year_end): array
	{
		$data=[];
		$this->load->model('bus_model');
		$data['name'] = 'Bus';
	...
	private function monthly_sightseeing_search_history_log(string $year_start, string $year_end): array
	{
		$data=[];
		$this->load->model('sightseeing_model');
	...
	private function monthly_transfer_search_history_log(string $year_start, string $year_end): array
	{
		$data=[];
		$this->load->model('transferv1_model');
		$data['name'] = 'Transfers';
...
private function monthly_car_search_history_log(string $year_start, string $year_end): array
	{
		$data=[];
		$this->load->model('car_model');
		$data['name'] = 'Cars';
		$temp_data = $this->car_model->monthly_search_history($year_start, $year_end);
		$data['data'] = $this->distribute_monthly_values($temp_data);
		$data['color'] = '#456F13';
		return $data;
	}
