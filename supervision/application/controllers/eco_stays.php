...
class Eco_stays extends CI_Controller
{
    ...
    function stays(string $origin = ''): void
    {
        ...
        if (empty($post_params) == false) {
            ...
            if (isset($insert_data['host'])) {
                $currency_id = $this->custom_db->single_table_records("user", "currency", array("user_id" => $insert_data['host']))['data'][0]['currency'];



                $insert_data['currency'] = ($currency_id > 0) ? $currency_id : 22;
            }
            ...
            if ($origin > 0) {
                ...
                if ($data['status'] == true) {
                    ...
                    if (isset($page_data['form_data']['amenities'])) {
                       ...
                       // debug( $page_data['form_data']['amenities']);die;
                    }
        ...
        //$total_records = $this->module_model->eco_stays_list($condition, true);
        //debug($total_records);die;

        $data_list = $this->db->query("SELECT eco_stays.*,CONCAT(host.first_name, ' ', host.last_name) AS host_name FROM eco_stays AS eco_stays LEFT JOIN user AS host ON host.user_id = eco_stays.host ORDER BY eco_stays.created_on DESC");


        // $this->load->library('pagination');
        // $config['base_url'] = base_url().'index.php/eco_stays/stays';
    ...
    public function delete_eco_stays(int $origin = 0): void
    {
        $origin = intval($origin);
        if ($origin > 0) {
            $data = $this->custom_db->single_table_records('eco_stays', '*', array('origin' => $origin));
            if ($data['status'] == true) {
                ...
                $this->custom_db->delete_record('eco_stays_gallery_images', array('stays_origin' => $origin));

                //delete seasons 
                $this->custom_db->delete_record('eco_stays_seasons', array('stays_origin' => $origin));

                //delete rooms
                $this->custom_db->delete_record('eco_stays_rooms', array('stays_origin' => $origin));

                //delete reviews
                $this->custom_db->delete_record('eco_stays_admin_reviews', array('stays_origin' => $origin));

                //delete theme map
                $this->custom_db->delete_record('eco_stays_theme_map', array('stays_origin' => $origin));

                //delete room availability
                $this->custom_db->delete_record('eco_stays_room_availability', array('stays_origin' => $origin, 'booked =' => 0));
    ...
    public function gallery_images(string $stays_origin = ''): void
    {
        $config = [];
        $insert_data = [];
        $stays_origin = provab_decrypt($stays_origin);
        $stays_origin = intval($stays_origin);
        $page_data = array();
        if ($stays_origin > 0) {
            $stays_data = $this->custom_db->single_table_records('eco_stays', '*', array('origin' => $stays_origin));
    ...
    public function delete_gallery_images(string $stays_origin = '', int $origin = 0): void
    {
        $page_data = [];
        $origin = intval($origin);
        if ($origin > 0) {
            $page_data['origin'] = $origin;
            $data = $this->custom_db->single_table_records('eco_stays_gallery_images', '*', array('origin' => $origin));
            if ($data['status'] == true) {
                // DELETE Image FILES
    ...
    function seasons(string $stays_origin = '', string $origin = ''): void
    {
        $page_data = [];
        $stays_origin = provab_decrypt($stays_origin);
        $origin = provab_decrypt($origin);
        // debug($stays_origin);die;
        // echo "dfdfdf";die;
        $stays_origin = intval($stays_origin);
        $origin = intval($origin);
        if ($stays_origin > 0) {
            ...
            if ($stays_data['status'] == true) {
                ...
                if (empty($post_params) == false) {
                    ...
                    // debug($valid_start_and_end_date);die;
                ...
                $this->template->view('eco_stays/seasons', $page_data);
            } else {
                set_error_message('UL00100');
                redirect('eco_stays/seasons/' . $stays_origin);
                exit();
    ...
    function partial_pay(string $stays_origin = ''): void
    {
        $page_data = [];
        $stays_origin = provab_decrypt($stays_origin);
        //$stays_origin = intval($stays_origin);
            ...
            if ($stays_data['status'] == true) {
                ...
                if (empty($post_params) == false) {
                    ...
                    if (isset($post_params['origin'])) {
                       ...
                       if ($insert_data['partial_value'] < 0 || $insert_data['partial_value'] > 100 || !is_numeric($insert_data['partial_value']) || floor($insert_data['partial_value']) != $insert_data['partial_value']) {
    
                            $this->session->set_flashdata('failed', "Value Must be between 0 to 100");
                            redirect('eco_stays/partial_pay/' . $origin);
                        } else {
                            if ($data['status'] == true) {
                               
                                $this->custom_db->update_record('eco_stays', $insert_data, array('origin' => $origin));
					...
					refresh();
                        ...
                        //invalid request
                        set_error_message('UL00100');
                        redirect('eco_stays/partial_pay/' . $stays_origin);
                        exit();
                    }
                }
            }
            $data_list = $this->custom_db->single_table_records('eco_stays', '*', array('origin' => $stays_origin));
            if ($data_list['status'] == true) {
                $page_data['data_list'] = $data_list['data'];
                $page_data['stay_origin'] = $stays_origin;
            }
            $this->template->view('eco_stays/partial_pay', $page_data);
        } else {
            set_error_message('UL00100');
            redirect('eco_stays/partial_pay/' . $stays_origin);
            exit();
    ...
    function delete_seasons(string $stays_origin = '', int $origin = 0): void
    {
        $stays_origin = intval($stays_origin);
        if ($stays_origin > 0) {
            $stays_data = $this->custom_db->single_table_records('eco_stays', '*', array('origin' => $stays_origin));
    ...
    function rooms(string $stays_origin = '', int $origin = 0): void
    {
        $page_data = [];
        $stays_origin = provab_decrypt($stays_origin);
        $origin = provab_decrypt($origin);
    ...
    function delete_rooms(string $stays_origin = '', int $origin = 0): void
    {
        $stays_origin = intval($stays_origin);
        if ($stays_origin > 0) {
            $stays_data = $this->custom_db->single_table_records('eco_stays', '*', array('origin' => $stays_origin));
    ...
    function room_price_management(string $room_origin = '', string $origin = ''): void
    {
        $room_origin = provab_decrypt($room_origin);
        $origin = provab_decrypt($origin);
        //$room_origin_re = $room_origin;
        // echo "dfdf";die;
        $room_origin = intval($room_origin);
        $origin = intval($origin);
        $page_data = array();
        // debug($room_origin);die;
        ...
        if ($room_data['status'] == true) {
           ...
           // $stay_data = $this->custom_db->single_table_records("eco_stays", "*", array("origin" => $page_data['room_data']['stays_origin']));
            $page_data['data_list'] = $room_prices;
            $query = "select CC.country as currency from eco_stays as ES INNER JOIN user AS U ON ES.host = U.user_id INNER JOIN currency_converter as CC ON U.currency=CC.id where ES.origin=" . $page_data['room_data']['stays_origin'];
            $run_query = $this->db->query($query)->result_array()[0]['currency'];
            $page_data['room_currency'] = $run_query;
            $page_data['room_currency']='USD';
    ...
    function room_availability_calendar(string $room_origin = ''): void
    {
        $room_origin = provab_decrypt($room_origin);
        $room_origin = intval($room_origin);
        $page_data = array();
    ...
    function delete_room_price(string $room_origin = '', int $origin = 0): void
    {
        $room_origin = intval($room_origin);
        $origin = intval($origin);
    ...
    function room_cancellation_policy(string $room_origin = '', string $origin = ''): void
    {
        $room_origin = provab_decrypt($room_origin);
        $origin = provab_decrypt($origin);
        $room_origin = intval($room_origin);
        ...
        if ($room_data['status'] == true) {
            ...
            $page_data['room_currency'] = $run_query;
            // set_supplier_currency($run_query);
    ...
    function delete_room_cancellation_policy(int $room_origin = 0, int $origin = 0): void
    {
        $room_origin = intval($room_origin);
        if ($room_origin > 0) {
            $room_data = $this->custom_db->single_table_records('eco_stays_rooms', '*', array('origin' => $room_origin));
    ...
    private function check_cancellation_days_validity(int $room_origin = 0, int $origin = 0, int $to_before_days): bool
    {
        ...
        if (empty($cancellation_policies) == false) {
            $existing_upto_days = array_column($cancellation_policies['to_before_days']);
        }
    ...
    public function types(string $origin = ''): void
    {
        error_reporting(0);
        $page_data = array();
        $page_data['origin'] = 0;
    ...
    public function room_types($origin = '')
    {
        $page_data = array();
        $page_data['origin'] = 0;
    ...
    public function board_types(string $origin = ''): void
    {
        $page_data = array();
        $page_data['origin'] = 0;
    ...
    public function delete_board_types(int $origin = 0)
    {
        $page_data = [];
        $origin = intval($origin);
        if ($origin > 0) {
    ...
    public function eco_stays_amenities(string $origin = ''): void
    {
        $page_data = array();
        $page_data['origin'] = 0;
        $config = [];
    ...
    public function delete_eco_stays_amenities(int $origin = 0)
    {
        $page_data =[];
        $origin = intval($origin);
        if ($origin > 0) {
    ...
    public function eco_stays_room_amenities(string $origin = ''): void
    {
    ...
    public function delete_eco_stays_room_meal_types(int $origin = 0)
    {
        $page_data = [];
        $origin = intval($origin);
        if ($origin > 0) {
    ...
    public function eco_stays_review_criteria(int $origin = 0)
    {
        $page_data = array();
        $page_data['origin'] = 0;
        $config = [];
    ...
    public function delete_eco_stays_review_criteria(int $origin = 0)
    {
        $page_data = [];
        $origin = intval($origin);
        if ($origin > 0) {
