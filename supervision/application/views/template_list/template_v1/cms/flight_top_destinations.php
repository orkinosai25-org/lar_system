...
function get_status_label(int $status): string
{
    if ((int) $status == ACTIVE) {
        return sprintf(
            '<span class="label label-success"><i class="fa fa-circle-o"></i> %s</span>
            <a role="button" href="" class="hide">%s</a>',
            get_enum_list('status', ACTIVE),
            get_app_message('AL0021')
        );
    }

    return sprintf(
        '<span class="bg-red-active"><i class="fa fa-circle-o"></i> %s</span>
        <a role="button" href="" class="hide">Deactivate</a>',
        get_enum_list('status', INACTIVE)
    );
}

function get_status_toggle_button(int $status, int $origin): string
{
    if ((int) $status == ACTIVE) {
        return sprintf(
            '<a role="button" href="%sindex.php/cms/deactivate_flight_top_destination/%s" class="text-danger">Deactivate</a>',
            base_url(),
            $origin
        );
    }
    ...
    return sprintf(
        '<a role="button" href="%sindex.php/cms/activate_flight_top_destination/%s" class="text-success">Activate</a>',
        base_url(),
        $origin
    );
}
