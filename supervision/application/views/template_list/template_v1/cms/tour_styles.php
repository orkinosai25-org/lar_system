...
<div class="bodyContent">
	<div class="row">
		<div class="pull-left" style="margin:5px 0">
			<a href="<?=base_url().'index.php/cms/add_tour_styles'?>">
				<button  class="btn btn-primary btn-sm pull-right amarg">Add Tour Style</button>
			</a>
		</div>
	</div>
			...
			<table class="table table-condensed">
				...
				?>
					<tr>
						...
						<td><img src="<?php echo $GLOBALS ['CI']->template->domain_tour_style_images ($v['image']) ?>" class="img-thumbnail" height="100px" width="100px"></td>
						<td><?=get_status_toggle_button($v['status'], $v['origin'])?></td>
						<td><?=get_edit_button($v['origin'])?> <?php echo " "?>
								<a href="<?php echo base_url(); ?>cms/delete_tour_styles/<?php echo $v['origin']; ?>/<?php echo $v['origin']; ?>"  data-original-title="Delete"  onclick="return confirm('Do you want delete this record');" class="btn btn-danger btn-xs has-tooltip" data-original-title="Delete"> 
...
{
	$url = base_url() . 'index.php/cms/add_tour_styles?' . $_SERVER['QUERY_STRING'] . '&origin=' . $origin;
...
$(document).ready(function() {
	$('.toggle-user-status').on('change', function(e) {
		...
		if (parseInt(_user_status) == 1) {
			_opp_url = _opp_url+'activate_tour_style/';
		} else {
			_opp_url = _opp_url+'deactivate_tour_style/';
		}
