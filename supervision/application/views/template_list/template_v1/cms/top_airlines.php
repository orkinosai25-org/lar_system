...
<div class="bodyContent">
	<div class="row">
		<div class="pull-left" style="margin:5px 0">
			<a href="<?=base_url().'index.php/cms/add_top_airlines	'?>">
				<button  class="btn btn-primary btn-sm pull-right amarg">Add Top Airlines</button>
			</a>
		</div>
	</div>
		...
		<div class="panel-body">
			<table class="table table-condensed">
				...
				?>
					<tr>
						<td><?=($k+1)?></td>
						<td><?=$v['airline_name']?></td>
						<td><img src="<?php echo $GLOBALS ['CI']->template->domain_top_airline_images ($v['logo']) ?>" class="img-thumbnail" height="100px" width="100px"></td>
						<td><?php echo get_status_toggle_button($v['status'],$v['origin']) ?></td>
						<td><?=get_edit_button($v['origin'])?> <?php echo " "?>
							<a href="<?php echo base_url(); ?>cms/delete_top_airline/<?php echo $v['origin']; ?>/<?php echo $v['origin']; ?>"  data-original-title="Delete"  onclick="return confirm('Do you want delete this record');" class="btn btn-danger btn-xs has-tooltip" data-original-title="Delete"> 
...
{
	$url = base_url() . 'index.php/cms/add_top_airlines?' . $_SERVER['QUERY_STRING'] . '&origin=' . $origin;
	$label = get_app_message('AL0022');

	return '<a role="button" href="' . $url . '" class="btn btn-sm btn-primary">
		<i class="fa fa-edit"></i> ' . $label . '</a>';
}
...
$(document).ready(function() {
	$('.toggle-user-status').on('change', function(e) {
		...
		if (parseInt(_user_status) == 1) {
			_opp_url = _opp_url+'activate_top_airline/';
		} else {
			_opp_url = _opp_url+'deactivate_top_airline/';
		}
