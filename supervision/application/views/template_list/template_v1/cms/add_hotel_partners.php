<?php
   ...
   // debug($get_data);exit;
 	if(isset($get_data) && valid_array($get_data)){
 		$action = base_url().'index.php/cms/add_hotel_partners?&origin='.$get_data['origin'];
 	}
 	else{
 		$action = base_url().'index.php/cms/add_hotel_partners';
 	}
 	// echo $action;exit;
?>
<!-- HTML BEGIN -->
...
<ul class="nav nav-tabs" role="tablist" id="myTab">
	...
	<li role="presentation" class="<?=$tab1?>">
		<a id="fromListHead" href="#fromList" aria-controls="home" role="tab"	data-toggle="tab">Manage Hotel Partners</a>
	</li>
      ...
      <div class="panel-body">
         <form class="form-horizontal" role="form" enctype="multipart/form-data" method="POST" action="<?= $action; ?>" autocomplete="off" name="home_page_heading">
            <fieldset form="promo_codes_form_edit">
               <legend class="form_legend">Add Hotel Partner</legend>
               
               <?php if(isset($partner_image)){?>
               <div class="form-group">
                  <label form="promo_codes_form_edit" for="module" class="col-sm-3 control-label">Current Image</label>
                  <div class="col-sm-6">
                    <img src="<?php echo $GLOBALS ['CI']->template->domain_hotel_partner_images ($partner_image) ?>" height="100px" width="100px" class="img-thumbnail">
               ...
               <?php } ?>
				<input type="hidden" value="1" name="origin">
              	<div class="form-group">
                  <label form="promo_codes_form_edit" for="partner_image" class="col-sm-3 control-label">Partner Image</label>
                  	<div class="col-sm-6">
                  		<input type="file" id="partner_image" class="form-control" name="partner_image" value="<?php echo @$partner_image?>" <?php if(!isset($partner_image)){?>required<?php } ?>>
                  	</div>
               </div>
              </fieldset>
            <div class="form-group">
