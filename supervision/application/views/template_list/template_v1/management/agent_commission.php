...
function get_table($table_data='')
{
	$table = '';
	$filter_agency = '';
	if(isset($_GET['filter_agency'])){
		$filter_agency = $_GET['filter_agency'];
	}
	 if (isset($GLOBALS['CI']->pagination)) : ?>
    <?= $GLOBALS['CI']->pagination->create_links(); ?>
<?php endif;
	$search_filter = '<div class="">
					...
					<input type="text" autocomplete="off" placeholder="Search" class="form-control ui-autocomplete-input" id="filter_agency" name="filter_agency" value="'.$filter_agency.'">
					</div>
...
function update_commission_button($agent_id)
{
	return '<a role="button" href="'.base_url().'index.php/management/agent_commission?agent_ref_id='.base64_encode($agent_id).'" class="btn btn-sm btn-primary"><i class="fa fa-plus-square"></i> 
...
function flight_commission_tab($flight_comm_details, $agent_ref_id, $super_admin_flight)
{
	$flight_com_origin = 0;
	$flight_com_value = 0;
	if(isset($flight_comm_details['origin']) == true && intval($flight_comm_details['origin']) > 0) {
		$flight_com_origin = $flight_comm_details['origin'];
		$flight_com_value = $flight_comm_details['value'];
	}
	$comm = $super_admin_flight['api_value'] + 0;
	$details = '
        ...
        <div class="panel-body commission_wrapper">
	        ...
	        <input type="hidden" name="commission_origin[]" value="'.intval($flight_com_origin).'">
	        <input type="hidden" name="commission[]" value="'.floatval($flight_com_value).'">
	        </div>
            <div class="col-md-12">
	               Agents Get 
	               <select name="api_value[]" class="api_value" data-superadmin_commission="'.floatval($super_admin_flight['api_value']).'">
	               	<option value="0">0</option>
	               	'.generate_options(numeric_dropdown(array('size' => ($super_admin_flight['api_value']*2), 'divider' => 2)), array(floatval(($flight_comm_details['api_value'])))).'
	               </select>
	             	% 
