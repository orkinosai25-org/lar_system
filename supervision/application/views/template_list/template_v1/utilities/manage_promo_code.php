...
function get_table($promo_code_list)
{
	...
	if(valid_array($promo_code_list)) {
		...
		foreach($promo_code_list as $v) {
			...
			$action = '';
			extract($v);
			if(intval(strtotime($v['expiry_date'])) <= 0) {
				$validity_label  = 'Unlimited';
				$days_left = 100;//TO Enable Edit Option, Setting Days left to 100
			} else {
				$days_left = get_date_difference(date('Y-m-d'), $v['expiry_date']);
				if($days_left < 0) {
					$days_left = 0;
				}
				$validity_label = app_friendly_absolute_date($v['expiry_date']).'('.$days_left.' Days Left)';
			}
			if($days_left > 0) {
				$action .= get_edit_button($v['origin']);
				
				$action .= share_button($v['origin']);
			} else {
				$action = 'Validity Expired';
				$action .= get_delete_buttoon($v['origin']);
			}
			$table .= '<tr>';
			$table .= '<td>'.(++$current_record).'</td>';
			$table .= '<td>'.$v['promo_code'].'</td>';
			$table .= '<td> '.$image.'</td>';
			$table .= '<td>'.$v['value'].'  '.get_enum_list('value_type',$v['value_type']).'</td>';
			$table .= '<td>'.$validity_label.'</td>';
			$table .= '<td>'.$v['minimum_amount'].'</td>';
			$table .= '<td>'.ucfirst($v['module']).'</td>';
			$table .= '<td>'.get_status_label($v['status']).'</td>';
