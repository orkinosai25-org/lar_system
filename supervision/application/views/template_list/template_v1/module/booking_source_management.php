                  ...
                  <div class="">
                     <?php 
                        ...
                        if (isset($eid) == false || empty($eid) == true) {
                           ...
                           echo $this->current_page->generate_form('booking_source_management', $form_data);
                        } else {
                           echo $this->current_page->generate_form('booking_source_management_edit', $form_data);
                        }
   ...
   <th>'.get_app_message('AL0012').'</th>
	...
	if (valid_array($table_data) == true) {
		foreach ($table_data as $k => $v) {
			$action = get_edit_button($v['origin'], $v['source_id']);
			
			$table .= '<tr>
			<td>'.($k+1).'</td>
			<td>'.$v['name'].'</td>
			<td>'.$v['source_id'].'</td>
			<td>'.$v['meta_course_name'].'</td>
			<td>'.get_status_label($v['booking_engine_status']).'</td>
...
{
		return '<a role="button" href="'.base_url().'index.php/module/booking_source_management?eid='.$id.'&course_id='.$course_id.'" class="btn btn-default btn-sm"><i class="fa fa-edit"></i>
		</a>
		';
