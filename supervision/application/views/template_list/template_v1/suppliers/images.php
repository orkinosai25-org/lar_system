													...
													<div class='form-actions' style='margin-bottom: 0'>
														<div class='row'>
															<div class='col-sm-9 col-sm-offset-3'>
																...
																<a href="<?php echo base_url(); ?>index.php/supplier/view_with_price" class="btn btn-primary">Go Back</a>
															</div>
														</div>
													</div>
												</form>
                                      ...
                                      <?php } else { ?>
                                          <select
																	onchange="activate(this.value);">
																		<option
																			value="<?php echo base_url() ?>index.php/supplier/update_traveller_image_status/<?php echo $travel->package_id; ?>/<?php echo $travel->img_id; ?>/1">Activate</option>
																		<option
																			value="<?php echo base_url() ?>index.php/supplier/update_traveller_image_status/<?php echo $travel->package_id; ?>/<?php echo $travel->img_id; ?>/0"
																			selected>De-activate</option>
																</select>
                                      <?php } ?>
                        </td>
																<td class="center"><a
																	href="<?php echo base_url() ?>index.php/supplier/delete_traveller_img/<?php echo $travel->img_id; ?>/<?php echo $travel->package_id; ?>"
...
<script type="text/javascript">
    ...
    $('#photo').on('change', function () {
        ...
        var maxSize = 1 * 1024 * 1024; // Maximum size in bytes (1 MB)

        if (file) {
            if (file.size > maxSize) {
                $('#error-message').text('The file is too large. Maximum size is 1 MB.').show();
                $('#photo').val(''); // Clear the input field
            } else {
                $('#error-message').hide();
            }
        }
