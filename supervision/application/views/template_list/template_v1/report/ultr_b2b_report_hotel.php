...
<div class="bodyContent col-md-12">
	<div class="panel panel-default clearfix"><!-- PANEL WRAP START -->
			<div class="panel-heading"><!-- PANEL HEAD START -->
				<?=$GLOBALS['CI']->template->isolated_view('report/ultra_report_tab_b2b')?>
            ...
            <div class="clearfix"></div>
                <div class="dropdown col-xs-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="excel_imp_drop">
                        <li >
                            <a href="<?php echo base_url(); ?>index.php/report/export_confirmed_booking_hotel_report_b2b/excel<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">Confirmed Booking</a>
                        </li>
                        <li>
                            <a href="<?php echo base_url(); ?>index.php/report/export_cancelled_booking_hotel_report_b2b/excel<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">Cancelled Booking</a>
                        </li>
                    </ul>
                </div>
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
		    ...
		    foreach($booking_details as  $parent_v) { 
		       //debug($parent_v);exit;
				...
				$action = '';
				//$tdy_date = date ( 'Y-m-d' );
				$diff = get_date_difference($parent_v['tdy_date'],$parent_v['hotel_check_in']);
				$customer_details = customer_details($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status']);
				$action .= hotel_voucher($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status'],'b2b');
				$action.='<br/>';
				$action .= hotel_pdf($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status']);
				$action.='<br/>';
				$action .= hotel_voucher_email($parent_v['app_reference'], $parent_v['booking_source'],$parent_v['status'],$parent_v['email']);
				$action .= '<br />' . $customer_details;
                $action .= hotel_GST_Invoice($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status'], 'b2b');
				$action.='<br/>';
		    	if($parent_v['status'] == 'BOOKING_CONFIRMED' && $diff > 0) {
					$action .= cancel_hotel_booking($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status']);
				}
				...
				$action .= hotel_email_voucher($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status']);
			
		$report_data .= '<tr>
					<td>'.($current_record++).'</td>
					<td>'.$parent_v['agency_name'].'</td>
					<td>'.$parent_v['app_reference'].'</td>
					<td class="">'.$parent_v['confirmation_reference'].'/<br/>'.$parent_v['booking_reference'].'</span></td>
					<td>'.$parent_v['lead_pax_name']. '<br/>'.
						  $parent_v['lead_pax_email'].'<br/>'.
						  $parent_v['$lead_pax_phone_number'].'
					</td>
					<td>'.$parent_v['hotel_name'].'</td>
					<td>'.$parent_v['total_rooms'].'<br/>('.$parent_v['adult_count'].'+'.$parent_v['child_count'].')</td>
					<td>'.$parent_v['hotel_location'].'</td>
					<td>'.date('d-m-Y', strtotime($parent_v['hotel_check_in'])).'/<br/>'.date('d-m-Y', strtotime($parent_v['hotel_check_out'])).'</td>
					<td>'.$parent_v['currency'].''.($parent_v['fare']).'</td>
					<td>'.$parent_v['currency'].''.$parent_v['admin_markup'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['agent_markup'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['gst'].'</td>
					<td>'.$parent_v['currency'].''.($parent_v['fare'] + $parent_v['admin_markup']+$parent_v['gst']).'</td>
					<td>'.$parent_v['currency'].''.round($parent_v['fare'] + $parent_v['admin_markup'] + $parent_v['agent_markup']+$parent_v['gst']).'</td>
					<td>'.date('d-m-Y', strtotime($parent_v['voucher_date'])).'</td>
					<td><span class="'.booking_status_label($parent_v['status']).'">'.$parent_v['status'].'</span></td>
