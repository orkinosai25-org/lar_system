            ...
            <div class="clearfix"></div>
                <div class="dropdown col-xs-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="excel_imp_drop">
                        <li >
                            <a href="<?php echo base_url(); ?>index.php/report/export_confirmed_booking_hotel_report/excel<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">Confirmed Booking</a>
                        </li>
                        <li>
                            <a href="<?php echo base_url(); ?>index.php/report/export_cancelled_booking_hotel_report/excel<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">Cancelled Booking</a>
                        </li>
                    </ul>
                </div>
	...
	$report_data .= '<table class="table table-condensed table-bordered" id="b2c_report_hotel_table">
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
		    ...
		    foreach($booking_details as $parent_v) { 
				...
				$action .= hotel_voucher($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status'],'b2c');
				$action.='<br/>';
				...
				$action .= '<br />' . $customer_details;
                                $action .= hotel_GST_Invoice($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status'], 'b2c');
				$action.='<br/>';
		...
		$report_data .= '<tr>
					...
					<td>'.$parent_v['currency'].''.($parent_v['itinerary_details'][0]['TDS']).'</td>
					<td>'.$parent_v['currency'].''.$parent_v['admin_markup'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['convinence_amount'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['gst'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['discount'].'</td>
					<td>'.$parent_v['currency'].''.$parent_v['grand_total'].'</td>
					...
					<td><span class="'.booking_status_label($parent_v['payment_details'][0]['status']).'">'.$parent_v['payment_details'][0]['status'].'</span></td>
