            ...
            <div class="panel-title">
                <ul class="nav nav-tabs nav-justified master_tabs" role="tablist" id="myTab">
                    ...
                    <li role="presentation" class="<?php echo $tab1; ?>"><a id="fromListHead" href="#fromList"
                            aria-controls="home" role="tab" data-toggle="tab"> Create/Update Type </a></li>
                    <li role="presentation" class="<?php echo $tab2; ?>"><a href="#tableList" aria-controls="profile"
                            role="tab" data-toggle="tab"> Types List </a></li>
                ...
                <div role="tabpanel" class="tab-pane <?php echo $tab1; ?>" id="fromList">
                    <div class="panel-body">
                        ...
                        if ($origin > 0) {
                            ...
                            // echo '<img width="20%" src="' . base_url() . $GLOBALS['CI']->template->domain_d_pckg_images_upload_dir('types/' . $form_data['image']) . '">';

                            echo '<img width="10%" src="'.base_url().$GLOBALS['CI']->template->domain_eco_stays_images_upload_dir('types/' .$form_data['image']).'">';
    ...
    if (valid_array($table_data) == true) {
       ...
       //debug($table_data);die;
        foreach ($table_data as $v) {
			...
			<td>' . $v['name'] . '</td>';
if ($v['image']!='') {
          $table .= '<td><img width="10%" src="'.$test.'types/' .$v['image'].'"></td>';
} else {
	   $table .= '<td></td>';
}
           $table .= '<td>' . get_enum_list('status', $v['status']) . '</td>									
...
{
    return '<a role="button" href="' . base_url() . 'index.php/eco_stays/types/' . provab_encrypt($id) . '" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i>
		' . get_app_message('AL0022') . '</a>';
}

function get_delete_button($id)
{
    return '<a role="button" href="' . base_url() . 'index.php/eco_stays/delete_types/' . $id . '" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i>Delete</a>';
}
?>
...
$(document).ready(function(){
  $("#types_reset").click(function(){
    location.replace("https://www.travelsoho.com/LAR/supervision/index.php/eco_stays/types");
  });
});
</script>
<script>
