...
Class Private_Management_Model extends Abstract_Management_Model
{
	 ...
	 */
	function airline_domain_markup(): array
	{
		$response=[];
		$response['data'] = '';
	 ...
	 * Manage domain markup for provab - Domain wise and module wise
	 */
	function hotel_domain_markup(): array
	{
		$response=[];
		$response['data'] = '';
	 ...
	 * Manage domain markup for provab - Domain wise and module wise
	 */
	function transfer_domain_markup(): array
	{
		$response=[];
		$response['data'] = '';
	 ...
	 * Manage domain markup for provab - Domain wise and module wise
	 */
	function sightseeing_domain_markup(): array
	{
		$response=[];
		$response['data'] = '';
	 ...
	 * Manage domain markup for provab - Domain wise and module wise
	 */
	function bus_domain_markup(): array
	{
		$response=[];
		$response['data'] = '';
	 ...
	 * @param $markup_level
	 */
	function generic_domain_markup(string $module_type): array
	{
	 ...
	 */
	function specific_domain_markup(string $module_type): array
	{
	 ...
	 */
	function master_transaction_request_list(string $type = 'provab'): array
	{
	 ...
	 */
	function process_balance_request(int $origin, string $system_request_id, string $status_id, string $update_remarks): array
	{
		$response=[];
...
/**
	 ...
	 */
	function update_domain_balance(int $domain_origin, float $amount): float
	{
		$current_balance = 0;
	 ...
	 * @param number $amount		amount to be added or deducted(-100 or +100)
	 */
	function update_b2b_balance(int $origin, float $amount): float
	{
		$current_balance = 0;
	 ...
	 * @param number $amount		amount to be added or deducted(-100 or +100)
	 */
	function update_b2b_debit_balance(int $origin, float $amount): float
	{
		// echo $amount;exit;
		$current_balance = 0;
		$cond = array('user_oid' => intval($origin));
	 ...
	 */
	function update_b2b_credit_limit(int $origin, float $amount): float
	{
		// $current_balance = 0;
		$cond = array('user_oid' => intval($origin));
	 ...
	 *
	 */
	function event_logs(bool $count = false, int $offset = 0, int $limit = 100000000000): array
	{
		//BT, CD, ID
		if ($count) {
