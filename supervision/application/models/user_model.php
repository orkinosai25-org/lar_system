...
class User_Model extends CI_Model
{
	 ...
	 */
	public function get_user_details(array $condition = [], bool $count = false, int $offset = 0, int $limit = 10000000000, array $order_by = []): array
	{
	 ...
	 * get Domain user list in the system
	 */
	function b2b_user_list(array $condition = [], bool $count = false, int $offset = 0, int $limit = 10000000000, array $order_by = []): array|object
	{
	 ...
	 * @param $user_id_list
	 */
	function account_ledger_summary(array $user_id_list): array
	{
		// $user_dep_summ = array();
	 ...
	 * @param $user_id_list
	 */
	function car_booking_summary(array $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
		...
		//
		$query = 'SELECT created_by_id, status, count(*) AS request_count from car_booking_details Where ' . $user_id_cond . ' Group BY created_by_id, status ';
	 ...
	 * @param $user_id_list
	 */
	function flight_booking_summary(array $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
	 ...
	 * @param $user_id_list
	 */
	function hotel_booking_summary(array $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
	 ...
	 * @param $user_id_list
	 */
	function transfer_booking_summary(int $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
	 ...
	 * @param $user_id_list
	 */
	function sightseeing_booking_summary(int $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
	 ...
	 * @param $user_id_list
	 */
	function bus_booking_summary(int $user_id_list): array
	 ...
	 * @param mixed $user_id_list
	 */
	function user_deposit_summary(array $user_id_list): array
	{
		// $user_dep_summ = array();
		$user_id_cond = '';
		if (valid_array($user_id_list) == true) {
	 ...
	 */
	function get_logged_in_users(array $condition = [], bool $count = false, int $offset = 0, int $limit = 10000000000): array|object
	{
		$filter_condition = ' and ';
		if (valid_array($condition) == true) {
	 ...
	 */
	function get_domain_details(): array
	{
	 ...
	 *@return status;
	 */
	function update_login_manager(int $S_LID = 0): void
	{
		$condition = array(
	 ...
	 */
	function create_login_auth_record(string $user_id, int $user_type, int $user_origin = 0, string $username = 'customer'): int
	{
		$data=[];
		$login_details=[];
	 ...
	 * @param $browser
	 */
	function update_auth_record_expiry(string $user_id, int $user_type, string $remote_ip, int $user_origin, string $username): void
	{
		$cond=[];
	 ...
	 */
	//sms configuration
	function sms_configuration(string $sms): object
	{
	...
	function fb_network_configuration(string $social): string
	{
	...
	function google_network_configuration(string $social): string
	{
		$social_links = $this->db_cache_api->get_active_social_network_list();
		return isset($social_links[$social]) ? $social_links[$social]['config'] : false;
	}
	...
	function sms_checkpoint(string $name): mixed
	{
	...
	public function get_subscribed_emails(int $domain_key, ?string $email = null): array
	{
		if (isset($email)) {
	 ...
	 */
	function update_subscribed_emails(array $data, array $cond): array
	{
		$response=[];
	 ...
	 * update user information
	 */
	function update_user_data(array $data, array $cond): array
	{
		$response=[];
	 ...
	 * Balu A
	 */
	function get_admin_user_id(): array
	{
		$cond=[];
		$admin_user_id = array();
	 ...
	 */
	function get_agent_info(int $user_id): array
	{
	...
	function get_agent_info_ultra(int $user_id): array
	{
		$query = 'select U.*,BU.logo,BU.origin,BU.balance, BU.credit_limit, BU.due_amount,BU.origin as Bid from user AS U
					      join  b2b_user_details BU on U.user_id = BU.user_oid
				          join  currency_converter CUC on CUC.id = BU.currency_converter_fk
						  WHERE  U.user_type=' . ULTRALUX_USER . ' AND U.user_id=' . $user_id;
		return $this->db->query($query)->result_array();
	}

	function get_admin_details(array $data): array|bool
	{
	...
	function get_privilage_list(int $user_id, string $filter_text = ''): array
	{
	...
	function edit_user_privileges(int $user_id, int $privileges_origin): void
	{
		$previlages=[];
		...
		if ($this->custom_db->delete_record('privileges', array(
			...
			foreach ($privileges_origin as $v) {
