...
Class Car_Model extends CI_Model implements report_model
{
	...
	//----------------------sdp ------- 
public function b2c_car_report(array $condition = [], bool $count = false, int $offset = 0, int $limit = 100000000000): array|string
	{
		$response=[];
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records
					from car_booking_details BD
					left join car_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code
					left join user as U on BD.created_by_id = U.user_id 
					where (U.user_type='.B2C_USER.' OR BD.created_by_id = 0) AND BD.domain_origin='.get_domain_auth_id().' '.$condition.'';
...
public function b2b_car_report(array $condition = [], bool $count = false, int $offset = 0, int $limit = 100000000000): array|string
	{
	 $response=[];
		$condition = $this->custom_db->get_custom_condition($condition);
	...
	public function ultra_car_report(array $condition = [], bool $count = false, int $offset = 0, int $limit = 100000000000): array|string
	{
	 $response=[];
		$condition = $this->custom_db->get_custom_condition($condition);
		...
		if ($count) {
			$query = 'select count(distinct(BD.app_reference)) as total_records 
					...
					join car_booking_itinerary_details AS HBID on BD.app_reference=HBID.app_reference
					join payment_option_list AS POL on BD.payment_mode=POL.payment_category_code 
					left join user as U on BD.created_by_id = U.user_id  where U.user_type='.ULTRALUX_USER.' AND BD.domain_origin='.get_domain_auth_id().' '.$condition;
			$data = $this->db->query($query)->row_array();
			return $data['total_records'];
		} else {
			$this->load->library('booking_data_formatter');
  ...
  public function get_booking_details(string $app_reference, string $booking_source, string $booking_status = ''): array
	...
	public function get_monthly_booking_summary(array $condition = []): array
	{
		$condition = $this->custom_db->get_custom_condition($condition);
	 ...
	 */
	public function update_cancellation_refund_details(string $AppReference, array $cancellation_details): void
	{
	...
	{
		$query = 'select count(*) AS total_search, MONTH(created_datetime) as month_number from search_car_history where
	...
	{
		$query = 'select count(*) AS total_search,  from_location as label from search_car_history where
		(YEAR(created_datetime) BETWEEN ' . $year_start . ' AND ' . $year_end . ') AND domain_origin=' . get_domain_auth_id() . ' 
		
		GROUP BY CONCAT(from_location, to_location) order by count(*) desc, created_datetime desc limit 0, 15';
		//debug($query );exit;
		return $this->db->query($query)->result_array();
	}
