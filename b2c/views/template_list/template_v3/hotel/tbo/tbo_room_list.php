 ...
 */
function generate_combinations(array $data, array &$all = [], array $group = [], $value = null, int $i_val = 0): array
{
    ...
    $currentKey = $keys[$i_val];
    $currentElement = $data[$currentKey];

    foreach ($currentElement as $val) {
        generate_combinations($data, $all, $group, $val, $i_val + 1);
    }
...
$common_params_url .= '<input type="hidden" name="hotel_code"				value="'.$_HotelRoomsDetails[0]['HOTEL_CODE'].'">';
...
foreach ($_RoomCombinations['RoomCombination'] as $__rc_key => $__rc_value) {
	...
	$room_panel_details = '';
	$room_panel_summary = $dynamic_params_url = array();//SUPPORT DETAILS
	...
	foreach ($current_combination_wrapper as $__room_index_key => $__room_index_value) {
		...
		// debug($temp_room_details['HOTEL_CODE']);exit;
		if( $params['booking_source']=='PTBSID0000000011'){
			//echo "hello ";die;
		$data1 = $this->custom_db->single_table_records('eco_stays_gallery_images', '*', array('stays_origin' => $temp_room_details['HOTEL_CODE']));
			$temp_room_details['RoomImage']='https://www.travelsoho.com/LAR/extras/custom/TMX1512291534825461/uploads/eco_stays/stays/'.$data1['data'][0]['image'];

}
		$rateplancode = json_decode($temp_room_details['RoomTypeCodeD'], true);
		$check_val = true;

		if(isset($params['RoomTypeCode'])){
		
			if($rateplancode[0] == $params['RoomTypeCode']){
				$check_val = false;
			}
		}
		if($check_val){
		$common_params_url .= '<input type="hidden" name="CancellationPolicy[]"	value="'.$temp_room_details['CancellationPolicy'].'">';//Balu A
		$common_params_url .= '<input type="hidden" name="roomTypeCode"	value="'.$rateplancode[0].'">';//Balu A
		$room_panel_details .='<div class="rm_img"><img src="'.$temp_room_details['RoomImage'].'" alt="img/jpg"/></div>';
		$room_panel_details .='<div class="col-sm-4 col-xs-6 nopad full_mobile hide">';
		...
		$dynamic_params_text = serialized_data($dynamic_params_url);
		$hotel_code = preg_replace("/[^a-zA-Z0-9]/", "",$temp_room_details['HOTEL_CODE']);
		$temp_dynamic_params_url = '';
		$temp_dynamic_params_url .= '<input type="hidden" name="token" value="'.$dynamic_params_text.'">';
		$temp_dynamic_params_url .= '<input type="hidden" name="token_key" value="'.md5($dynamic_params_text).'">';
		if($adult_count>1){
		...
		$room_panel_details .='</div>';	
		$room_panel_details .='<div class="rm_detl"><h4>'.ucfirst(strtolower($temp_room_details['RoomTypeName'])).'</h4>';
		$room_panel_details .='<div class="rm_dtl_btm"><div class="rm_prc"><span>Total for '.$no_of_nights.' '.$night_str.'</span><strong>'.$currency_obj->get_currency_symbol($currency_obj->to_currency).' '.$temp_price_details['RoomPrice'].'</strong></div>';
		
		$room_panel_details .='<div class="rm_btn"><a data-target="#room_details" data-toggle="modal"  id="roomamenties_'.$hotel_code.'_'.$__rc_key.'" class="room_amenties"  data-hotel-code='.$hotel_code.' data-room-name="'.$temp_room_details['RoomTypeName'].'" data-key="'.$__rc_key.'" data-room-code="'.$temp_room_details['RoomTypeCode'].'"
		data-room-price="'.$temp_price_details['RoomPrice'].'" data-room-amenties="'.base64_encode(json_encode($temp_room_details['Amenities'])).'" data-otherroom-amenties=""
		data-booking-source="'.$params['booking_source'].'" href="#room_details">View Amenities and Policies</a>';
		$room_panel_details .='<form method="POST" action="'.$booking_url.'">
	'.$common_params_url.$temp_dynamic_params_url.'<input type="submit" class="btn btn-default rm_book" name="room_button" value="Book Now" /></form></div>';
		$room_panel_details .='</div>';
		$room_panel_details .='</div>';	
		$room_panel_details .='  <div class="col-sm-4 col-xs-6 nopad full_mobile hide">';
		$room_panel_details .='  <div class="romsfst">';
	...
	if(valid_array($dynamic_params_url)){
	...
	$temp_book_link = '<div class="col-sm-4 col-xs-12 nopad full_mobile hide">
	...
	$clean_room_list .= '<div class="lst_in">';
...
<div class="modal fade room_amnty" id="room_details" role="dialog">
    <div class="modal-dialog">    
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Room amenities & policies :</h4>
        </div>
        <div class="modal-body">
			<p class="room-loader hide"><?=$mini_loading_image?></p>
			<div id="room_details_body">	
...
<script>
	...
	$(".room_amenties").on("click",function(){
		//$("#room_details").html('');
		var Hotel_code = $(this).data('hotel-code');
		var room_key = $(this).data('key');
		var Room_code = $("#roomamenties_"+Hotel_code+'_'+room_key).data('room-code');
		var Room_Name =  $("#roomamenties_"+Hotel_code+'_'+room_key).data('room-name');
		...
		var RoomAmenties = $("#roomamenties_"+Hotel_code+'_'+room_key).data('room-amenties');
		var OtherRoomAmenties = $("#roomamenties_"+Hotel_code+'_'+room_key).data('otherroom-amenties');
		
		$(".room-loader").removeClass('hide');
		$(".loader-image").removeClass('hide');
		$.ajax({
				url:"<?php echo base_url()?>"+'index.php/ajax/get_room_facilities',		
				data:{
					search_id:Search_id,policy_code:Policy_code,rate_key:Rate_key,
					room_price:Room_price,non_refundable:Non_refundable,booking_source:Booking_Source,tb_search_id:TB_search_id,currency_symbol:Currency_symbol,policy_details:Policy_details,today_cancel_date:Today_Cancel,room_code:Room_code,hotel_code:Hotel_code,room_name:Room_Name,room_amenties:RoomAmenties,otherroomamenties:OtherRoomAmenties
				},
				success:function(res){					
					//if (res.hasOwnProperty('status') == true && res.status == true) {
						$("#room_details_body").html(res.data);
						$(".room-loader").addClass('hide');
