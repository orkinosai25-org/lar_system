					...
					<?php
							for($ex_pax_index=1; $ex_pax_index <= $total_pax_count; $ex_pax_index++) {//START FOR LOOP FOR PAX DETAILS
								...
								if($pax_type != 'infant'){ ?>
										...
										<div class="pref_pax_name meal_pref_pax_name"><?php echo ucfirst($pax_type)?> <?php echo ($pax_type_count)?></div>
								...
								for($ex_pax_index=1; $ex_pax_index <= $total_pax_count; $ex_pax_index++) {//START FOR LOOP FOR PAX DETAILS
									...
									if($pax_type != 'infant'){
										...
										if($ex_pax_index == 1){
											...
											?>
											<span class="formlabel"> <?php echo $meal_p_label?></span>
								<?php 	} ?>
									<div class="col-xs-12 nopad spllty">
										<div class="selectedwrap">
											<select name="meal_pref<?php echo $meal_p_input_counter?>[]" class="mySelectBoxClass flyinputsnor">
												<option value="">Meal Preference</option>
												<?php foreach($meal_pov as $meal_pk => $meal_pv){ ?>
													<option value="<?php echo $meal_pv['MealId']?>"><?php echo $meal_pv['Description']?></option>
												<?php }
												?>
											</select>
										</div> 
					...
					<?php
							for($ex_pax_index=1; $ex_pax_index <= $total_pax_count; $ex_pax_index++) {//START FOR LOOP FOR PAX DETAILS
								...
								if($pax_type != 'infant'){ ?>
										...
										<div class="pref_pax_name seat_pref_pax_name" ><?php echo ucfirst($pax_type)?> <?php echo ($pax_type_count)?></div>
						...
						for($ex_pax_index=1; $ex_pax_index <= $total_pax_count; $ex_pax_index++) {//START FOR LOOP FOR PAX DETAILS
							...
							if($pax_type != 'infant'){
								...
								if($ex_pax_index == 1){
									...
									?>
									<span class="formlabel"> <?php echo $seat_pref_label?></span>
						<?php 	} ?>
					
						<div class="col-xs-12 nopad spllty">
							<div class="selectedwrap">
								<select name="seat_pref<?php echo $seat_p_input_counter?>[]" class="mySelectBoxClass flyinputsnor">
									<option value="">Seat Preference</option>
									<?php foreach($seat_pov as $seat_k => $seat_v){ ?>
										<option value="<?php echo $seat_v['SeatId']?>"><?php echo $seat_v['Description']?></option>
									<?php }
									?>
								</select>
							</div>
