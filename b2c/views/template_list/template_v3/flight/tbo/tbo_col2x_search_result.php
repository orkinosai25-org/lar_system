...
foreach ($flights_data as $__tirp_indicator => $__trip_flights) {
    ...
    foreach ($__trip_flights as $__trip_flight_k => $__trip_flight_v) {
        ...
        //Api price details
		$adult_price = $__trip_flight_v['PassengerFareBreakdown']['ADT']['TotalPrice'];
         //debug($adult_price);exit;
        ...
        foreach ($cur_SegmentDetails as $__segment_k => $__segment_v) {
            ...
            //Way Summary in one line - End
               ...
               // debug($__segment_flight);exit;
                ...
                //Between Content -----
                $inner_summary .= '<div class="col-xs-2 nopad5 hide">';
                $inner_summary .= '</div>';
                ...
                <div class="layoverdiv">
                    <div class="centovr">
                    <span class="fa fa-plane"></span>Change Of Planes ' . $waiting_time.' Layover in '.$next_seg_info['OriginDetails']['CityName'].'
                </div></div>
                <div class="clearfix"></div>';
                }
            }
...
//print_r($count);
            ...
            if (str_ends_with($dur[0], 'm')) {
                $check = (int)$dur[0];  // extract minutes
            } else {
                $check = (int)$dur[0] * 60;  // assume it's in hours
            }
            ...
            //airline
            $outer_summary .= '<div class="quarter_wdth nopad ar_logo ' . add_special_class('col-xs-3', 'col-xs-3', $domestic_round_way_flight) . '">
            ...
            $outer_summary .= '<div class="smal_udayp nopad ' . add_special_class('col-xs-3', 'col-xs-3', $domestic_round_way_flight) . '"><span class="f-d hide">' . $duration . '</span>
                                            <div class="insidesame">
                                                ...
                                                <div class="stp_detl"><div class="stop-value">Stop:' . ($__stop_count) . '</div><div class="city_code1">'.$stop_air.'</div></div><div class="cabinclass hide">' . ($cabin_class) . '</div>
                                            </div>
                                        </div>';
            //arrival
            $outer_summary .= '<div class="col-xs-3 nopad quarter_wdth flt_dest">
                                            <div class="insidesame">
        ...
        $outer_summary .= '
                    <div class="col-xs-5 nopad wayfour full_same">';
         //Load Flight Details Button
        $outer_summary .= '<div class="mrinfrmtn">
                                    <a class="detailsflt iti-btn" data-id="fdp_' . $__root_indicator . $__trip_flight_k . '"><span class="fal fa-info-circle fldetail" data-toggle="tooltip" title="" data-original-title="Flight Itinerary"></span>View Flight Details '.$remark_separator.'</a><i>' . $cur_AirlineRemark .'</i><span class="ft_rfnd">'.$Refundable_lab.'
                            </span></div>';
                        ...
                        <div class="priceanbook">
                            <div class="col-xs-6 nopad wayprice">
                                <div class="insidesame">
                                    ...
                                    <span class="hide price" data-base_price="' . $api_base_price . '" data-tax_price="' . $api_tax_price . '" data-price="' . round($o_Total_Fare) . '" data-original_price="' . $api_total_price . '" data-original_markup="' . $original_markup . '" data-markup_type="' . $markup_type . '" data-api_offered_fare="' . $api_offered_fare . '" data-currency="' . $cur_Currency . '"></span>
                                    <div data-toggle="tooltip" data-placement="right" title="' . $Refundable_lab . '" data-val="' . intval($cur_IsRefundable) . '" class="n-r n-r-t hide"><i class="far fa-info-circle"></i></div>
                                </div>
                            </div>
                            <div class="col-xs-6 nopad waybook">
                                <div class="form-wrapper bookbtlfrt">';
                                /*<button class="b-btn bookallbtn" data-toggle="collapse" data-target="#select_fare">Select Fare</button>*/
                            $outer_summary.='<form method="POST" id="form-id-' . $__root_indicator . $__trip_flight_k . '" action="' . $booking_url . '" class="book-form-wrapper">
        ...
        $outer_summary .= '<div class="clearfix"></div>';
                        ...
                        /*<div id="select_fare" class="collapse slct_fare">
                            <div class="slct_detl">
                                <div class="col-xs-12 col-md-3 slct_lft">
                                    <ul>
                                        <li>Fare Types</li>
                                        <li>Baggage</li>
                                        <li>Change/Cancellation</li>
                                    </ul>
                                </div>
                                <div class="col-xs-12 col-md-3 slct_rght">
                                    <ul>
                                        <li class="slct_clas"><span>First Class <strong>$ 209</strong></span><button class="b-btn bookallbtn" type="submit">Book</button></li>
                                        <li class="slct_bg"><span>7 kg Cabin bag <br>20 kg Check-in bag</span></li>
                                        <li class="slct_cncl"><span>Cancellation charges upto $ 70 <br>Change charges upto $49</span></li>
                                    </ul>
                                </div>
                                <div class="col-xs-12 col-md-3 slct_rght">
                                    <ul>
 ...
 */
function add_special_class($col_2x_class, $col_1x_class, $domestic_flight) {
    return $domestic_flight ? $col_2x_class : $col_1x_class;
}
