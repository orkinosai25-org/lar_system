...
Class Module_Model extends CI_Model
{
	 ...
	 * Balu A
	 */
	function module_management($primary_key, $course_id)
	{
		$tmp_data = $this->db->query("
			...
			WHERE MCL.origin = $primary_key AND MCL.course_id = '$course_id' 
			GROUP BY BS.origin
		");
	...
	function get_active_payment_module_list()
	{
		...
		$payment_module = $this->db->query($query)->row_array();
		if (isset($payment_module['payment_category'])) {
			$payment_module = explode(DB_SAFE_SEPARATOR, $payment_module['payment_category']);
		}
	 ...
	 */
	function log_exception($module, $opvalue, $notification)
	{
		$data = [];
	 ...
	 * @param string $notification
	 */
	function flight_log_exception($module, $op_val, $notification)
	{
		$data = [];
		$data['exception_id'] = 'EID-'.time().'-'.rand(1, 100);
		$data['module'] = $module;
