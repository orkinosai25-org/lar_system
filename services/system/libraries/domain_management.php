...
class Domain_Management 
{
	...
	public function validate_domain($domain_user_details) 
	{
		...
		$system = trim($domain_user_details['System']);
              ...
              $serverIp= '184.171.160.86';
                // if($system!="live")
                // {
                // 	$data['message'] = 'Invalid System Value, Please send system as live';
                // 	exit;
                // }
		$domain_login = $this->CI->user_model->domain_login ($DomainKey, $UserName, $Password, $system, $serverIp);
		...
		if ($domain_login['status'] == SUCCESS_STATUS && valid_array($domain_login['data']) == true) {
			...
			$domain_details['bus_version3'] = 	$complete_domain_details['bus_version3'];
		...
		$domain_details['holiday_version'] =      $complete_domain_details['holiday_version'];
	...
	public function validate_domain_course_version($course, $course_version, $domain_details)
	{
		...
		switch($course){
			...
			case META_BUS_COURSE:
				...
				break;
		    case META_TBO_BUS_COURSE:
				$domain_course_version = $domain_details['bus_version3'];
				break;		
			...
			case META_CAR_COURSE:
				...
				break;
			case META_HOLIDAY_LIVE_COURSE:
                 $domain_course_version = $domain_details['holiday_version'];
				break;     
		}
		// if($_SERVER['REMOTE_ADDR'] == '184.95.42.178'){
		// 	echo 'course'.$course_version;
		// echo "<br/>";
		// echo $domain_course_version;exit;
		// }
	...
	public function get_flight_commission($domain_origin)
	{
		if($this->is_domain_user == true) {
			...
			//echo $flight_commission_query;exit;
	 ...
	 * @param unknown_type $credential_type
	 */
        public function credit_domain_balance_hold_booking($transaction_amount, $credential_type, $domain_origin=0, $attr=array())
	{
