...
class Booking_Data_Formatter {
	 ...
	 */
	function format_cancel_hotel_booking_data($complete_booking_details, $module)
	{
		//debug($complete_booking_details);exit;
		$currency_obj = new Currency();
		$formatted_booking_details = array();
		$new_key = 0;
		foreach($complete_booking_details as $key => $book_v) {	
			$cancellation_date = '';
			$cancellation_details = array();
			if($book_v['status'] == "BOOKING_CANCELLED"){
				$cancellation_details = $GLOBALS['CI']->custom_db->single_table_records('hotel_cancellation_details', '*', array('app_reference' => $book_v['app_reference'], 'refund_status' => "PROCESSED"));
				if($cancellation_details['status'] == SUCCESS_STATUS){
					$cancellation_date = $cancellation_details['data'][0]['created_datetime'];
					$formatted_booking_details[$new_key] = $book_v;
					$formatted_booking_details[$new_key]['admin_currency'] = $book_v['currency'];
			    	$formatted_booking_details[$new_key]['total_fare'] = $book_v['total_fare'];
					$formatted_booking_details[$new_key]['domain_markup'] = $book_v['domain_markup'];
					$formatted_booking_details[$new_key]['refund_status'] = $cancellation_details['data'][0]['refund_status'];
					$formatted_booking_details[$new_key]['refund_amount'] = $cancellation_details['data'][0]['refund_amount'];
					$formatted_booking_details[$new_key]['refund_date'] = $cancellation_details['data'][0]['refund_date'];
					$formatted_booking_details[$new_key]['cancellation_charge'] = $cancellation_details['data'][0]['cancellation_charge'];
					...
					if(is_domain_user() == true){
						$formatted_booking_details[$new_key]['total_fare'] =  $this->add_currencyrate($book_v['total_fare'],$book_v['currency_conversion_rate']);
						$formatted_booking_details[$new_key]['total_fare'] =  $this->add_currencyrate($book_v['total_fare'],$book_v['currency_conversion_rate']);
						$formatted_booking_details[$new_key]['domain_markup'] =  $this->add_currencyrate($book_v['domain_markup'],$book_v['currency_conversion_rate']);
						$formatted_booking_details[$new_key]['level_one_markup'] =  $this->add_currencyrate($book_v['level_one_markup'],$book_v['currency_conversion_rate']);
						
						$formatted_booking_details[$new_key]['admin_net_fare'] = $this->add_currencyrate($admin_net_fare,$book_v['currency_conversion_rate']);
						$formatted_booking_details[$new_key]['agent_net_fare'] = $this->add_currencyrate($agent_net_fare,$book_v['currency_conversion_rate']);
					
						$currency = domain_base_currency();
					} else {
						$formatted_booking_details[$new_key]['admin_net_fare'] = $admin_net_fare;
						$formatted_booking_details[$new_key]['agent_net_fare'] = $agent_net_fare;
					}
					$formatted_booking_details[$new_key]['cancellation_date'] = $cancellation_date;	
					$formatted_booking_details[$new_key]['currency'] = $currency;

					$new_key++;
				}
			}
			
			// $currency_obj->get_currency_symbol($currency);//FIXME: Remove the hardcoded value "INR" Later
		}
		return $formatted_booking_details;
	}
	/**
	 * Jaganath
	 * @param array hotel booking_details
	...
	function format_hotel_booking_data($complete_booking_details, $module)
	{
		...
		foreach($complete_booking_details as $key => $book_v) {	
			$cancellation_date = '';
			if($book_v['status'] == "BOOKING_CANCELLED"){
				$cancellation_details = $GLOBALS['CI']->custom_db->single_table_records('hotel_cancellation_details', '*', array('app_reference' => intval($book_v['app_reference'])));
				if($cancellation_details['status'] == SUCCESS_STATUS){
					$cancellation_date = $cancellation_details['data'][0]['created_datetime'];
				}
			}
			$complete_booking_details[$key]['admin_currency'] = $book_v['currency'];
				...
				$complete_booking_details[$key]['cancellation_date'] = $cancellation_date;	
	...
	function format_hotel_booking_datas($complete_booking_details, $module)
	{
	   ...
	   //echo debug($master_booking_details);exit;
		$itinerary_details = $this->format_hotel_itinerary_details($complete_booking_details['data']['booking_itinerary_details']);
	...
	function format_flight_refund_booking_data($complete_booking_details, $module){
		$response['status']	= SUCCESS_STATUS;
		$response['data']	= array();
		$master_booking_details = $complete_booking_details['data']['booking_details'];
		$itinerary_details = $this->format_itinerary_details($complete_booking_details['data']['booking_itinerary_details']);	
		$ticket_dtails = $this->format_ticket_details($complete_booking_details['data']['booking_ticket_details']);
		$booking_details = array();
		foreach($master_booking_details as $book_k => $book_v) {
			$core_booking_details = $book_v;

			$core_booking_details['lead_pax_name'] = $core_booking_details['title'].' '.$core_booking_details['first_name'].' '.$core_booking_details['last_name'];
			$app_reference = $core_booking_details['app_reference'];
			...
			if(isset($ticket_dtails[$book_v['origin']])){
				$ticket_number = $ticket_dtails[$book_v['origin']]['TicketNumber'];
			}
		
		$pax_type = '';
		if($book_v['passenger_type'] == 'Adult'){
		    $pax_type = 'ADT';
		}
		if($book_v['passenger_type'] == 'Child'){
		    $pax_type = 'CHD';
		}
		if($book_v['passenger_type'] == 'Infant'){
		    $pax_type = 'INF';
		}
		$api_fare = json_decode($book_v['fare_breakup'], true);
		$api_fare_breakup = $api_fare['PassengerBreakup'][$pax_type];
		//debug($api_fare);exit;
		$total_api_fare_breakup = $api_fare['PassengerBreakup'];

		$pax_count = $api_fare_breakup['PassengerCount'];
		$total_base_fare = $api_fare_breakup['BasePrice']/$pax_count;
		$total_tax = $api_fare_breakup['Tax']/$pax_count;
		$total_pax_count = 0;
		foreach($total_api_fare_breakup as $fare_breakup){
		    //debug($fare_breakup);exit;
		    $total_pax_count+= $fare_breakup['PassengerCount'];
		}
		$total_api_fare = $total_base_fare+$total_tax;
		
			//debug($book_v);exit;
		$core_booking_details['booking_api'] = $book_v['booking_api_name'];
		$core_booking_details['total_fare'] = $total_api_fare;
		
		$core_booking_details['booking_id'] =  $book_v['book_id'];
		$core_booking_details['pnr'] =  $book_v['PNR'];
		$core_booking_details['TicketNumber'] = $ticket_number;
			...
			if(isset($transaction_details[$app_reference])) {
				...
				//check the module
					if(is_domain_user() == true){
					 ...
					 $agent_tds = 0;
					 $admin_tds = 0;
					 $negative_markup_tds=0;
					 $negative_tds_dedcuted_markup=0;
					 $pnr = '';
					 $booking_id = '';
					 $booking_api = '';
					 $fare_breakup = array();
		          ...
		          foreach($booking_transaction_details as $transaction_k => $transaction_v) {
                	...
                	$get_breakup_details=json_decode($transaction_v['fare_breakup'], true);

                    if(valid_array($pax_fares['Fare'])==false)
                	{
                       foreach($pax_fares as $key=>$pvalue)
                       {
                       	if($pvalue['passenger_type']=="Adult")
                       	{
                       		$PassengerType="ADT";
                       	}
                       	if($pvalue['passenger_type']=="Child")
                       	{
                       		$PassengerType="CHD";
                       	}
                        if($pvalue['passenger_type']=="Infant")
                       	{
                       		$PassengerType="INF";
                       	}

                    $PassengerCount=$get_breakup_details['PassengerBreakup'][$PassengerType]['PassengerCount'];
                    $PBasicFare=$get_breakup_details['PassengerBreakup'][$PassengerType]['BasePrice']/$PassengerCount;
                	$PTax=$get_breakup_details['PassengerBreakup'][$PassengerType]['Tax']/$PassengerCount;
                	$PTotalPrice=$get_breakup_details['PassengerBreakup'][$PassengerType]['TotalPrice']/$PassengerCount;

    	// if($_SERVER["REMOTE_ADDR"]=="106.198.117.192")
 //{
 	if(valid_array($get_breakup_details)==false)
 	{
 		$PBasicFare=0;
 		$PTax=0;
 		$PTotalPrice=0;

 		foreach($customer_details[$transaction_v['origin']] as $c_value)
 		{

 			$PBasicFare=$c_value['Fare']['BasePrice'];
 			$PTax=$c_value['Fare']['Tax'];
 			$PTotalPrice=$c_value['Fare']['TotalPrice'];
 		}
 		
 	}
 //}

                       	$pax_fares[$key]['Fare']['BasePrice']=$PBasicFare;
                       	$pax_fares[$key]['Fare']['Tax']=$PTax;
                       	$pax_fares[$key]['Fare']['TotalPrice']=$PTotalPrice;
                       }
                	}
						...
						$tr_negative_markup_tds=0;
							$tr_negative_markup_tds1=0;
							$tr_admin_tds_deducted_markup=0;
							$tr_admin_markup_new_tds=0;
						if($tr_admin_markup < 0)
						{

							$tr_negative_markup_tds=(($tr_admin_markup*5)/100);
							$tr_negative_markup_tds1=(($tr_negative_markup_tds*5)/100);

						# inclduing TDS of negative Markup Calculation 
                        $tr_admin_tds_deducted_markup = $transaction_v['domain_markup']-$tr_negative_markup_tds+$tr_negative_markup_tds1;
                        $tr_admin_markup_new_tds=(($tr_admin_tds_deducted_markup*5)/100);
	...
	//if($_SERVER['REMOTE_ADDR']=="223.182.230.248") {
$bt_domain_markup_tds=0;
$bt_domain_markup_tds1=0;
$bt_admin_tds_deducted_markup=0;
$bt_admin_markup_new_tds=0;
							if($booking_transaction_details[$transaction_k]['domain_markup'] < 0)
						{
							$bt_domain_markup_tds=(($booking_transaction_details[$transaction_k]['domain_markup']*5)/100);
							$bt_domain_markup_tds1=(($bt_domain_markup_tds*5)/100);

						# inclduing TDS of negative Markup Calculation 
                        $bt_admin_tds_deducted_markup = $booking_transaction_details[$transaction_k]['domain_markup']-$bt_domain_markup_tds+$bt_domain_markup_tds1;
                        $bt_admin_markup_new_tds=(($bt_admin_tds_deducted_markup*5)/100);

						}

				$booking_transaction_details[$transaction_k]['bt_admin_tds_deducted_markup']=$bt_admin_tds_deducted_markup;
				$booking_transaction_details[$transaction_k]['bt_admin_tds_deducted']=$bt_admin_markup_new_tds;
			//}
						...
						$negative_tds_dedcuted_markup += $tr_admin_tds_deducted_markup;
						$negative_markup_tds += $tr_admin_markup_new_tds;
						...
						if($transaction_v['booking_source'] == 'PTBSID0000000007'){
							...
							$check_time = strtotime('2018-11-13 15:00:00');
						...
						// $fare_breakup = $this->flight_fare_breakup($pax_fares, $tr_admin_markup, $tr_admin_comm, $tr_agent_comm, $tr_admin_tds, $tr_agent_tds, $currency_conversion_rate);
						// debug($fare_breakup);exit;
						/*if($_SERVER['REMOTE_ADDR']=="223.182.218.204")
						{
							echo $tr_fare.'|'.$tr_admin_markup.'|'.$tr_agent_comm.'|'.$tr_agent_tds.'<br />';
							echo $tr_fare + $tr_admin_markup- $tr_agent_comm + $tr_agent_tds;exit;
						}*/
						$booking_transaction_details[$transaction_k]['fare_breakup'] = $fare_breakup;
						$booking_transaction_details[$transaction_k]['api_fare_breakup'] = $get_breakup_details;
						...
						$booking_transaction_details[$transaction_k]['fare_breakup']['TotalNetFare'] = roundoff_number($tr_fare);
					}
			...
			$core_booking_details['negative_markup_tds_dedcuted'] = roundoff_number($negative_tds_dedcuted_markup);
			$core_booking_details['negative_markup_tds'] = roundoff_number($negative_markup_tds);
	 ...
	 * @param unknown_type $itinerary_details
	 */
	private function format_hotel_itinerary_details($itinerary_details)
	{
		$booking_itinerary_details = array();
		foreach($itinerary_details as $itinerary_k => $itinerary_v) {
	 ...
	 * @param unknown_type $itinerary_details
	 */
	private function format_ticket_details($ticket_details)
	{
		$booking_ticket_details = array();
		foreach($ticket_details as $ticket_k => $ticket_v) {
			$booking_ticket_details[$ticket_v['passenger_fk']] = $ticket_v;
		}
	 ...
	 * Implode Reference Ids
	 */
	function implode_passenger_fks($booking_details)
	
	{
		$passenger_fks = '';
		foreach($booking_details as $k => $v) {
			$passenger_fks .= '"'.$v['origin'].'",';
		}
		$passenger_fks = rtrim($passenger_fks, ',');
		return $passenger_fks;
	}
	...
	function display_currency_for_hotel($currency_rate = array(),$itinerary_details=array()){
		$conversion_rate = $currency_rate['currency_conversion_rate'];
    ...
    /**
	 * Anitha G
	 * @param array car booking_details
	 */
	function format_car_booking_data($complete_booking_details, $module)
	{
		$currency_obj = new Currency();
		foreach($complete_booking_details as $key => $book_v) {
		// debug($book_v);exit;	
			$complete_booking_details[$key]['admin_currency'] = $book_v['currency'];
			    $complete_booking_details[$key]['total_fare'] = $book_v['total_fare'];
				$complete_booking_details[$key]['domain_markup'] = $book_v['domain_markup'];
				$complete_booking_details[$key]['domain_gst'] = $book_v['domain_gst'];
				$currency = get_application_default_currency();
				
				$admin_net_fare = ($book_v['total_fare']);
				$agent_net_fare = ($book_v['total_fare']+$book_v['domain_markup']+$book_v['domain_gst']);
				
				if(is_domain_user() == true){
					$complete_booking_details[$key]['total_fare'] =  $this->add_currencyrate($book_v['total_fare'],$book_v['currency_conversion_rate']);
					$complete_booking_details[$key]['domain_markup'] =  $this->add_currencyrate($book_v['domain_markup']+$book_v['domain_gst'],$book_v['currency_conversion_rate']);
					$complete_booking_details[$key]['level_one_markup'] =  $this->add_currencyrate($book_v['level_one_markup'],$book_v['currency_conversion_rate']);
    ...
    function format_car_booking_datas($complete_booking_details, $module)
	{
		...
		foreach($master_booking_details as $book_k => $book_v) {
			...
			$admin_markup += $master_booking_details[0]['domain_markup']+$master_booking_details[0]['domain_markup'];
			$agent_markup = 0;
			...
			$booking_details['itinerary_details'] = $booking_itinerary_details;
			//Itenary Details
			$booking_details['extra_service_details'] = $complete_booking_details['data']['booking_extra_details'];
			//Customer Details
			$booking_details['customer_details'] = $booking_customer_details;
