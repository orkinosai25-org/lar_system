...
function is_active_car_module() {
	$status = false;
	if (is_domain_user () == false || in_array ( META_CAR_COURSE, $GLOBALS ['CI']->active_domain_modules )) {
		$status = true;
	}
	return $status;
}
...
function load_bus_lib_tbo($source, $obj = '', $object_name_as_class = false) 
{
	...
	$data = $CI->load->library ( 'bus_tbo/' . $folder_name . '/' . $class_name, $params, $obj_name );
...
function is_domain_user_master_admin() {
	...
	//$domain_id = @$GLOBALS ['CI']->entity_domain_id;
        ...
        if($entity_domain_id=="VH32773379" || $entity_domain_id=="VH3277452180" || $entity_domain_id=="VH327743527") {
        	return true;
	} else {
		return false;
	}
 ...
 */
function json_validate_old($json, $assoc = TRUE) {
...
function master_class_name($source) 
{
	...
	switch ($source) {
		...
		case MYSTIFLY_FLIGHT_BOOKING_SOURCE :
			$class_name = 'mystifly';
			break;
                case MYSTIFLY_LCC_FLIGHT_BOOKING_SOURCE :
			$class_name = 'mystiflylcc';
			break;    
                case TBO_HOTEL_BOOKING_SOURCE :
			$class_name = 'tbo';
			break;
		...
		case TRAVELPORT_MEELO_FLIGHT_BOOKING_SOURCE :
			...
			break; 
		case TBO_AIR_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'tboair';
			break;
		case AIRBLUE_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'airbluescrap';
			break;
		case AIRBLUE_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'airbluescrap';
			break;
		case CEBU_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'cebuscrap';
			break;
		case PK_FARE_FLIGHT_BOOKING_SOURCE :
			$class_name = 'pkfare';
			break; 
		case STTECH_AIRLINE_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'sttechairline';
			break;
                case TBO_BUS_BOOKING_SOURCE :
                        
                        $class_name = 'tbo';
			break;
                case GOFLYSMART_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'goflysmart';
			break;
                case RATEHAWK_HOTEL_BOOKING_SOURCE :
                        $class_name = 'ratehawk';
			break;
                case BOOKNTRAVEL_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'bookntravel';
			break;
                case TRIPJACK_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'tripjack';
			break;
		        case AIRIQ_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'airiq';
			break;
			case YATRA_HOTEL_BOOKING_SOURCE :
                        $class_name = 'YATRA';
			break;
			case MYSTIFLY_REST_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'mystiflyrest';
			break;
			case SABRE_REST_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'sabrerest';
			break;
			case YATRA_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'yatra';
			break;
			case DIDA_HOTEL_BOOKING_SOURCE :
                        $class_name = 'dida';
			break;
			case TRAVELOGY_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'travelogy';
			break;
			case HB_HOTEL_BOOKING_SOURCE :
                        $class_name = 'HB';
			break;
			case AIRIQ_API_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'airiqapi';
			break;
			case TRAVELOMATIX_FLIGHT_BOOKING_SOURCE :
                        $class_name = 'travelomatix';
			break;
...
// checkint the App Reference is duplicate or not
function check_app_reference($app_reference, $module) {
	$status = $GLOBALS ['CI']->module_model->check_app_reference ( $app_reference, $module );
	if ($status == ACTIVE) {
		return true;
	} else {
		return false;
