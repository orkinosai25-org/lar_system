...
function get_month_names()
{
	...
	//build months menu
	for ($m=1; $m<=12; $m++) {
		$month_names[($m-1)] = date('M', mktime(0,0,0,$m));
	}
	return $month_names;
}
...
Elavarasi
 */
function sightseeing_travel_dates($db_datetime)
{
	$db_datetime = strtotime ($db_datetime);
	if ($db_datetime > 0) {
		return date ("d|M|Y", $db_datetime );
...
function app_friendly_datetime($db_datetime)
{
	$db_datetime = strtotime($db_datetime);
	if ($db_datetime > 0) {
		return date('d-M-Y H:i a', $db_datetime);
	}
}
...
{
	return date('h:i A, D d - M',strtotime($date));
}
...
function getagefromdb($date_of_birth){
	$from = new DateTime($date_of_birth);
    $to   = new DateTime('today');
    $age = $from->diff($to)->y;
    $age = str_pad($age, 2, "0", STR_PAD_LEFT);
    return $age;
}