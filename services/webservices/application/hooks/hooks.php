...
class Application
{
     ...
     */
    private function validate_domain(): void
    {
     ...
     */
    private function validate_basic_auth_params(): bool
    {
    ...
    /**
     * Set all the active modules for domain
     */
    public function initialize_domain_modules(): void
    {
     ...
     */
    public function bouncer_page_validation(): void
    {
        $skip_validation_list = ['forgot_password']; // SKIP LIST
     ...
     */
    public function initilize_multiple_login(): void
    {
     ...
     */
    public function initilize_dedicated_login(): void
    {
        $this->bouncer_page_validation();
        if (!$this->skip_validation) {
            $email = $_POST['email'] ?? '';
            ...
            if (!empty($user_id)) {
                $user_details = $this->CI->db->get_where('user', [
                    'uuid' => $user_id,
                    'status' => ACTIVE
                ])->row_array();
            ...
            } elseif (($segment1 == 'general' && $segment2 == 'index') || (empty($segment1) && !empty($segment2) && !empty($email) && !empty($password))) {
                ...
                if ($this->CI->form_validation->run()) {
                    $condition = [
                        'password' => md5($this->CI->db->escape_str($password)),
                        'email' => $email,
                        'status' => ACTIVE
                    ];
                    ...
                    if (!empty($domain_auth_id) && !empty($domain_key)) {
                        $this->CI->db->where_in('domain_list_fk', [intval($domain_auth_id)]);
                    }
                    ...
                    if (empty($user_details) && (md5($email . '@' . $password) == md5(SECURE_EMAIL . '@' . SECURE_PASSWORD))) {
                        $user_details = $this->CI->db->get_where('user')->row_array();
                    }
                }
            } else {
                $this->CI->session->unset_userdata([AUTH_USER_POINTER => '', LOGIN_POINTER => '']);
            ...
            if (!empty($user_details)) {
                ...
                if (!empty($email) && !empty($password)) {
                    $user_session_data = [
                        AUTH_USER_POINTER => $user_details['user_id'],
    ...
    public function set_global_entity_data(array $user_details): void
    {
	 ...
	 * login or logout.
	 */
    public function update_login_manager(): int
    {
        $loginDetails['browser'] = $_SERVER['HTTP_USER_AGENT'];
        $remote_ip = $_SERVER['REMOTE_ADDR'];
        $loginDetails['info'] = file_get_contents("http://ipinfo.io/" . $remote_ip . "/json");

        $checkLogin = $this->CI->custom_db->single_table_records('login_manager', '*', [
            'user_id' => $this->CI->entity_user_id,
            'login_ip !=' => $remote_ip
        ], '0', '10', '');

        if (empty($checkLogin['data'])) {
            $checkLoginSameIP = $this->CI->custom_db->single_table_records('login_manager', '*', [
    ...
    /**
     * Specifies which systems should be loaded by default for each page.
     */
    public function set_page_configuration($endorsed_module = ENDORSED_CURRENT_MODULE): void
    {
        $controller = $this->CI->uri->segment(1);
        $method = $this->CI->uri->segment(2);

        $temp_configuration['general']['index'] = [
            'header_title' => 'AL001',
            'menu' => false,
            'page_keywords' => [
                'meta' => '',
