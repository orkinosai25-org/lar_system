$(document).ready(function() {
	...
	*/
	...
	//	$('input[name="passenger_passport_issuing_country[]"').change(function(){
		//console.log("pass");
...
//		}); 
	$('[type="submit"]').on('click', function(e) {
		...
		$('select:required').each(function() {
			 var id = $(this).attr('id');
			var ids_next = $("#" + id).next();
			//console.log("ids_next",ids_next);
			if (this.value == 'INVALIDIP') {
				if($(this).hasClass('invalid-ip')==false){
					$(this).addClass('invalid-ip').parent().append( 
					 "<span id='name_error' class='name_error'><div class='formerror'style='color:red'>This Field is mandatory</div></span>");
				}
				if (_status == true) {
					_status = false;
					_focus = this;
		...
		if(! validate_mobile($('#passenger-contact').val())) {
			...
			$('#passenger-contact').after('<p id="invalid_mobile_msg" class="text-danger">Mobile Number Should be 8 to 12 digits and This Field is mandatory</p>');
			_status = false;
			_focus = '';
		} else {
	...
	function validate_mobile(number){
		return /^[1-9][0-9]{7,11}$/.test(number);
			...
			popup_html += '<table class="table" id="passenger-confirm-table">';
				...
				$('#pre-booking-form').find('.pasngr_input').each(function(){
					...
					popup_html += '<tr>';
						...
						if($(this).find('#child-date-picker-'+passenger_count).is(':visible')){
						    	popup_html += '<td>';
	...
	*/
	...
	var grand_total = $("#actual_grand_total").val();
		var currency_symbolcc = $("#currency_symbol").val();
         ...
         success: 
              function(data){
				...
				if(data.status == 1){
					...
					$(".grandtotal").html(currency_symbolcc+' '+grand_total);
					$("#promocode_val").val('');
					$("#promo_code_discount_val").val(0);
					$("#promo_actual_value").val(0);
