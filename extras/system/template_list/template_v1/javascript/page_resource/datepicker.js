...
function futureDatepickerMonthDisabled(inputId) {
   //console.log(inputId);
    if (inputId.indexOf('m_flight_datepicker') >= 0) {
        var lastChar = inputId[inputId.length -1];
       // console.log(lastChar);
        ...
        onClose: function(dateStr) { 
             if(dateStr!=''){
                var ciDtSplt = dateStr.split("-");
              var checkInFrom = new Date(ciDtSplt[2], ciDtSplt[1] - 1, ciDtSplt[0]);
              var currentDay = checkInFrom.getDay();
              
              var dayNames = ["Sunday","Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];  
               $('.checkInDayDivMulti'+lastChar).text(dayNames[currentDay]); 
          }
        }
        });
    }
    if(inputId == "flight_datepicker1"){
        ...
        onClose: function(dateStr) { 
             if(dateStr!=''){
              ...
              var dayNames = ["Sunday","Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];  
               $('.checkInDayDiv').text(dayNames[currentDay]); 
          }
        }
        });
    }
    else if(inputId == "flight_datepicker2"){
        ...
        onClose: function(dateStr) { 
             if(dateStr!=''){
              ...
              var dayNames = ["Sunday","Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];  
               $('.checkOutDayDiv').text(dayNames[currentDay]); 
          }
        }
        });
