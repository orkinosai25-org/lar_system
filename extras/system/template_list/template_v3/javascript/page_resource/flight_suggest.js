...
$(document).ready(function() {
    ...
    function handle_active_trip_type(_active_trip_type) 
    {
        ...
        if (_active_trip_type == 'oneway') {
			$(".flight_chnge .fa-exchange").addClass('hide');
            $(".flight_chnge .fa-long-arrow-right").removeClass('hide');
            $('#flight_datepicker2').attr('disabled', true).removeAttr('required').closest('.date-wrapper').addClass('flt_togl');
            ...
            $('#multi_way_fieldset').removeClass('multi_show');
        } else if (_active_trip_type == 'circle') {
			 $(".flight_chnge .fa-exchange").removeClass('hide');
            $(".flight_chnge .fa-long-arrow-right").addClass('hide');
            ...
            $('#flight_datepicker2').removeAttr('disabled').attr('required', 'required').closest('.date-wrapper').removeClass('flt_togl').focus();
            $('#onw_rndw_fieldset').show();
        	$('#multi_way_fieldset').hide();
            $('#multi_way_fieldset').removeClass('multi_show');
            ...
            $('#multi_way_fieldset').addClass('multi_show');
    ...
    $(".fromflight").catcomplete({
        ...
        select: function(event, ui) {
            ...
            var airport_name=ui.item.airport_name;
            $('.airport_name_from').html(ui.item.airport_name);
            $('.from_loc_airport_name').val(ui.item.airport_name);
            $(this).siblings('.airport_value').html(airport_name);
            if (this.id == 'to') {
                to_airport = ui.item.value
                $('.terminal_to').html(ui.item.airport);
            } else if (this.id == 'from') {
                from_airport = ui.item.value
                $('.terminal_from').html(ui.item.airport);
            }
     ...
     $(".departflight").catcomplete({
        ...
        select: function (event, ui) {
            ...
            var category = ui.item.category;
            var airport_name=ui.item.airport_name;
             $('.airport_name_to').html(ui.item.airport_name);
             $('.to_loc_airport_name').val(ui.item.airport_name);
            $(this).siblings('.airport_value').html(airport_name);
            //console.log(label);
            ...
            if ($(this).hasClass('m_arrcity') == true && ui.item.value != '') {
                ...
                // if ($('#' + next_depcity_id).val() == '') {
                    $('#' + next_depcity_id).val(ui.item.value);
                    $('#' + next_depcity_id).siblings('.loc_id_holder').val(ui.item.id);
                    $('#' + next_depcity_id).siblings('.airport_value').html(airport_name);
                // }
            }
        },
        change: function (ev, ui) {
