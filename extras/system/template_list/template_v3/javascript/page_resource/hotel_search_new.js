$(document).ready(function() {
    ...
    function ini_map_view() {		
		...
		//var filter_count= $("#filter_records").text();

    	$.ajax({
			type: 'GET',
			url: app_base_url+'index.php/ajax/get_all_hotel_list/'+_offset+'?booking_source='+active_booking_source+'&search_id='+_search_id+'&op=load',
			async: true,
			cache: true,
			dataType: 'json',
			success: function(res) {
				$(".spinner").show();
				if(res.data.HotelSearchResult){
					var hotel_full_data = res.data.HotelSearchResult.HotelResults;					
					var objectCount = Object.keys(res.data.HotelSearchResult.HotelResults).length;									
					for (var i = 0; i < objectCount; i++){
						
						set_marker_visibility(hotel_full_data[i]);
    ...
    function get_map_attr_obj_old(thisRef, lat, lon, name, star)
    {
        var object = {};
	...
	function get_map_attr_obj(thisRef)
    {
        var object = {};
    ...
    function set_marker_visibility_old(thisRef, lat, lon, name, star)
    {
       ...
       //	console.log("lat"+lat);
        ...
        console.log("marker"+markers[access_key].length);
    }
  function set_marker_visibility(thisRef)
    {	
    ...
    function create_marker(obj, visibility)
    {
        ...
        //icon:api_url+'images/hotel_map_marker.png'
       // console.log(marker);
       // icon:api_url+'g_map_marker.png'
